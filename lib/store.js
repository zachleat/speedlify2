...
import { settleTime } from "./metrics.js";
import {
	SERIES_VERSION,
	buildSeries,
	hydratePoint,
...
/**
 * When `fastForwardAnimations()` landed, and whether a stored reading is new
 * enough to outlive the run that took it.
 *
 * A `difference`/`noJsText` pair taken before that fix measured the page's
 * animation rather than its scripts — see lib/axe.js. The numbers cannot be
 * recomputed from the image they came from, because that image is itself the
 * mid-fade frame, so a stale pair can only be dropped and taken again.
 *
 * Dropping matters because a pair does not expire on its own. A category set to
 * `screenshots: "none"` captures only for a site at full marks, so a site that
 * scored 400 once and has slipped since is never photographed again, and the
 * merge below would carry its pre-fix numbers forward forever. Two Eleventy
 * sites sat flagged that way. Without the pair the site is unjudged, which is
 * the honest answer for a reading nothing can confirm.
 *
 * A manifest with no `capturedAt` predates the stamp, which is only ever
 * written here — so it predates the fix as well.
 */
const ANIMATION_SETTLING_LANDED = Date.parse("2026-09-08T13:37:27Z");

function readingStillStands(manifest) {
	const at = Date.parse(manifest?.capturedAt ?? "");
	return !Number.isNaN(at) && at >= ANIMATION_SETTLING_LANDED;
}
...
export class ResultStore {
	...
	write(record) {
		...
		const { screenshots, pageShots, screenshotThrottling, screenshotMarkers, ...stored } = record;
		// The frames' own method, which is not the scored run's when a separate
		// throttled pass supplied them — see Runner.#measureOnce.
		this.writeFilmstrip(
			dir,
			screenshots,
			stored.timestamp,
			screenshotThrottling ?? stored.lab?.environment?.throttlingMethod ?? null,
			screenshotMarkers ?? null
		);
	 ...
	 *
	 * A reading taken before `fastForwardAnimations()` is not carried forward —
	 * see `readingStillStands`.
	 *
	...
	writeScreenshots(
		dir,
		{ primary = null, noJs = null, difference = null, noJsText = null, timestamp = null } = {}
	) {
		...
		const carried = readingStillStands(manifest);

		manifest = {
			...manifest,
			...(put(primary, "screenshot") ?? {}),
			noJs: wroteNoJs ?? manifest.noJs ?? null,
			// When the numbers below were taken, so a later change to how they are
			// taken can tell its own readings from the ones before it.
			capturedAt: wroteNoJs
			...
			difference: wroteNoJs ? difference : (difference ?? (carried ? manifest.difference : null) ?? null),
			...
			noJsText: wroteNoJs ? noJsText : (noJsText ?? (carried ? manifest.noJsText : null) ?? null),
		};
	 ...
	 */
	writeFilmstrip(dir, frames, timestamp, throttlingMethod = null, markers = null) {
		if (!frames?.length) return null;

		const framesDir = path.join(dir, "frames");
		fs.mkdirSync(framesDir, { recursive: true });

		// Computed here rather than at render time: it needs the decoded pixels,
		// which only exist at this point, and it is a constant of the measurement.
		const manifest = {
			timestamp,
			...jpegSize(frames[0].buffer),
			settledAt: settleTime(frames),
			// The Lighthouse setting these frames were captured under. Recorded
			// because it changes what they mean: "simulate" loads the page
			// unthrottled and estimates the metrics afterwards, so those frames are
			// a fast load, and holding them against a genuinely throttled strip
			// compares two different connections.
			throttlingMethod,
			// Observed TTFB, FCP and LCP on these frames' clock; only a separate throttled pass has them.
			markers,
			frames: [],
		};
		const keep = new Set();
