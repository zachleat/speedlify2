...
export function createComparator(options = {}) {
	...
	return function compare(a, b) {
		 ...
		 *
		 * A site with no CrUX coverage ranks below one that measurably passes and
		 * above one that measurably fails: not blamed for data that does not
		 * exist, but not credited with a pass it has not been shown to earn.
		 */
...
/**
 * 0 passing, 1 unsampled, 2 failing but nothing poor, 3 poor for real users.
 *
 * Unsampled sits between a pass and a failure, and only here. In the ring
 * counts above it is worth a green, because a site CrUX has never seen must not
 * show one fewer circle than a site it has — that would demote sites for being
 * small. But this step is not counting circles; it is asking what is known
 * about real users, and a site measurably passing for them has shown something
 * an unsampled site has not.
 *
 * Still ahead of any measured failure, on the same reasoning that keeps it out
 * of the band counts: no data is not evidence of a bad experience.
 */
function severityRank(entry) {
	const band = cwvBand(coreWebVitalFailures(entry), coreWebVitalWorst(entry) ?? "poor");
	if (band === "poor") return 3;
	if (band === "average") return 2;
	return band === "none" ? 1 : 0;
}
