...
export function bandCounts(entry, { useFieldData = true } = {}) {
	...
	const bands = [
		...
		// Off entirely when field data is switched out of the ranking, rather than
		// counted as a gray: a configuration that says "ignore CrUX" has to mean
		// the vital ring is not there at all, not that it is there and unknown.
		...(useFieldData ? [cwvBand(coreWebVitalFailures(entry), coreWebVitalWorst(entry) ?? "poor")] : []),
	];
	...
	return {
		 ...
		 *
		 * The vital is exempt: `cwvBand` returns "none" when CrUX has no sample,
		 * and that is a fact about a site's traffic rather than about the site.
		 * Most of this corpus is too small to be sampled.
		 */
		unchecked: bands.filter((b, i) => b === "none" && i < bands.length - (useFieldData ? 1 : 0)).length,
	};
}
...
export function createComparator(options = {}) {
	...
	return function compare(a, b) {
		...
		/*
		 * 1. The rings, counted by color: greens first, then ambers, then reds.
		 *
		 ...
		 *
		 * Counted rather than compared worst-first, which is what this used to do.
		 * Worst-first asked "who is showing the worst circle?" and only looked
		 * further on a tie; counting asks "who is showing the most green?" and
		 * settles ties downward. They part company when a site trades a red for
		 * extra greens, and counting is the rule the leaderboard states.
		 */
		...
		/*
		 * 2. Fewest accessibility violations — the node count, not the band.
		 *
		 * Directly under the rings, and the step that moved furthest. An amber axe
		 * ring costs nothing in the Lighthouse score, because axe is a separate
		 * run, so a site could carry four violations and a perfect 400 and beat a
		 * site whose only fault was the ring its total was docked for.
		 * preactjs.com sat six places above typescriptlang.org on exactly that.
		 *
...
/** 0 passing or unknown, 1 failing but nothing poor, 2 poor for real users. */
