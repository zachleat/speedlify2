...
export function createComparator(options = {}) {
	...
	return function compare(a, b) {
		/*
		 * A bot check is not a measurement.
		 *
		 * When a site serves Cloudflare's waiting room instead of itself, the run
		 * succeeds and the numbers describe the waiting room: a page of script and
		 * a spinner, which scores however it scores and has nothing to do with the
		 * site. Those numbers were being ranked — hawksworx.com sat at #1310 on
		 * the strength of "Just a moment...".
		 *
		 * Sorted with the unmeasured, which is what they are. The site keeps its
		 * row and its page, where the interstitial notice already explains why
		 * there are no figures; it simply stops being ordered against sites that
		 * were actually measured.
		 */
		const aMeasured = Boolean(a?.latest) && !a.interstitial;
		const bMeasured = Boolean(b?.latest) && !b.interstitial;
