...
export function projectPoint(record) {
	 ...
	 * gone.
	 *
	 * Only a meta tag's raw text is kept: DOM and header detections carry a bare
	 * version or header value there, which re-detection can't read back.
	 */
	const generator = record.axe?.generator;
	const tag = typeof generator === "string" ? generator : generator?.source === "meta" ? generator.raw : null;
	if (typeof tag === "string" && tag.trim()) point.gen = tag.slice(0, 120);
