...
import { confirmRedirect, isRetiredDestination, isLandingRedirect } from "./redirect.js";
import { detectHost, detectGenerator, detectTools, generatorById, challengedAxe, hiringStatus } from "./stack.js";
import companies from "../config/companies.js";
import { indexCompanies } from "./companies.js";
import {
	createComparator,
	rankLeaderboard,
	lighthouseSum,
...
import { domSnapshotFailed } from "./metrics.js";
...
/**
 * Pearson's r for a set of [x, y] pairs, with the n it was computed over.
 *
 * Null below eight pairs, and null when either axis is a single repeated value
 * — r is undefined there rather than zero, and printing 0.00 would read as
 * "no relationship" when the truth is "not enough to say".
 *
 * How to describe the number is left to whatever renders it. r is a measure of
 * how close the cloud sits to a straight line and nothing else: it says nothing
 * about the slope, and nothing whatever about cause.
 */
export function pearson(pairs) {
	...
	return { r: Math.round((cov / Math.sqrt(vx * vy)) * 100) / 100, n };
}
...
function buildScatter(entries, {
	...
	yScale = "log",
	yDomain = null,
	// The same two for the horizontal axis. A bounded score belongs on a linear
	// scale whichever axis it lands on — see `yScale` above for why.
	xScale = "log",
	xDomain = null,
	zeroColumn = false,
	zeroRow = false,
	...
	identity = false,
	/*
	 * Name every dot rather than only the green ones.
	 *
	 * The default withholds names from slow dots because most of this corpus is
	 * other people's personal sites, and labelling those turns a chart about a
	 * relationship into a list of who is worst. That reasoning does not reach the
	 * employees chart: its subjects are the largest companies on earth, named in
	 * their own category headings already, and a chart about corporate scale that
	 * hid which company each dot was would say nothing at all.
	 */
	nameAll = false,
	// What to call a dot, where the display URL is not the useful label.
	nameOf = (e) => e.displayUrl,
} = {}) {
	const points = [];
	for (const entry of entries) {
		...
		points.push({ bytes, lcp, name: nameOf(entry), slug: entry.slug });
	}
	 ...
	 */
	const linearX = xScale === "linear";
	const xv = (v) => (linearX ? v : Math.log10(v));
	const xs = points.filter((p) => linearX || p.bytes > 0).map((p) => xv(p.bytes));
	...
	let [x0, x1] = xDomain ? xDomain : pad(Math.min(...xs), Math.max(...xs));
	...
	// Only a log axis needs the break: on a linear one zero is an ordinary
	// coordinate, and carving a separate column for it would invent a gap in a
	// scale that does not have one.
	const zoneW = zeroColumn && zeroes && !linearX ? 54 : 0;
	...
	const zoneH = zeroRow && zeroRows && !linearY ? 34 : 0;
	...
	const sx = (bytes) =>
		bytes === 0 && zoneW
			? box.left + zoneW / 2
			: logLeft + ((xv(bytes) - domain.x0) / (domain.x1 - domain.x0)) * logW;
	const sy = (ms) =>
		ms === 0 && zoneH
	...
	// A count is a plain number with thousands separators — element counts here
	// run from 2 to 78,702, and "79 kB" would be a lie about what was measured.
	const count = (v) => Math.round(v).toLocaleString("en-US");
	const reader = { ms: secs, bytes, none: ratio, score, count };
	...
	const tickLabel = {
		...
		none: (v) => String(+v.toFixed(3)),
	};
	const xLabel = tickLabel[unit] ?? bytesLabel;
	// The vertical axis needs its own, now that it is not always a time. Labelling
	// DOM element counts with msLabel put "1 s" beside a thousand elements.
	const yLabel = tickLabel[unitY] ?? msLabel;

	const xTicks = [];
	if (linearX) {
		// Quarters, as on the linear vertical axis — see the yTicks branch below.
		const step = (domain.x1 - domain.x0) / 4;
		for (let i = 0; i <= 4; i++) {
			const v = domain.x0 + step * i;
			xTicks.push({ x: Math.round(sx(v)), label: xLabel(v) });
		}
	} else {
	...
	if (linearY) {
		...
		for (let i = 0; i <= 4; i++) {
			...
			yTicks.push({ y: Math.round(sy(v)), label: yLabel(v) });
		}
	} else {
		for (let e = Math.ceil(domain.y0); e <= Math.floor(domain.y1); e++) {
			yTicks.push({ y: Math.round(sy(10 ** e)), label: yLabel(10 ** e) });
		}
	}

	/*
	 * The legend, in the units of whichever axis the colors came from.
	 *
	 * Built here rather than written into the template, because one macro draws
	 * every scatter on the page and the boundaries are not the same on all of
	 * them: green is under 2.5 s on the paint charts, under 0.1 on the layout
	 * shift one, and 90 and above on the ones scored out of a hundred. Fixed LCP
	 * wording put "1,232 within 2.5 s" under a chart with no time axis on it.
	 *
	 * The same wording the threshold lines use, and a non-breaking space so a key
	 * never wraps between the number and its unit.
	 */
	const bandLabel = (v) =>
		(bandsAt || metric === "cls" ? String(v) : msLabel(v)).replace(" ", "\u00a0");
	// "877 90 and above" is not a sentence. A score is the one banded axis here
	// that needs a verb between the count and the range it falls in.
	const verb = (bandOn === "x" ? unit : unitY) === "score" ? "scoring " : "";
	const legend = (
		higherIsBetter
			? [
					["good", `${verb}${bandLabel(good)} and above`],
					["average", `${verb}${bandLabel(poor)} to ${bandLabel(good)}`],
					["poor", `${verb}under ${bandLabel(poor)}`],
				]
			: [
					["good", `within ${bandLabel(good)}`],
					["average", `to ${bandLabel(poor)}`],
					["poor", `over ${bandLabel(poor)}`],
				]
	).map(([band, label]) => ({ band, label, count: dots[band].length }));
	...
	return {
		...
		identity: identity
			? {
					x1: Math.round(sx(linearX ? domain.x0 : 10 ** domain.x0)),
					y1: Math.round(sy(linearY ? domain.y0 : 10 ** domain.y0)),
					x2: Math.round(sx(linearX ? domain.x1 : 10 ** domain.x1)),
					y2: Math.round(sy(linearY ? domain.y1 : 10 ** domain.y1)),
				}
			: null,
		count: points.length,
		/*
		 * Pearson's r over the coordinates as plotted — logs where the axis is
		 * logarithmic, raw values where it is linear.
		 *
		 * Correlating the untransformed numbers instead would answer a question
		 * nobody looking at the chart is asking: on axes spanning five decades, a
		 * single 53 MB page would set r almost by itself. This is the strength of
		 * the straight line the eye is already trying to draw.
		 *
		 * Points in the zero column or zero row are left out, having no coordinate
		 * on a log scale to correlate — `n` says how many were actually used, so a
		 * chart that excluded a third of its dots cannot quietly claim otherwise.
		 */
		correlation: pearson(
			points
				.filter((p) => (linearX || p.bytes > 0) && (linearY || p.lcp > 0))
				.map((p) => [xv(p.bytes), yv(p.lcp)]),
		),
		counts: { good: dots.good.length, average: dots.average.length, poor: dots.poor.length },
		legend,
		// The legend as one sentence, for the plot's own description. Assembled
		// here because the keys and the alt text must not be able to disagree.
		summary: legend
			.map((k, i) => `${i === legend.length - 1 ? "and " : ""}${k.count.toLocaleString("en-US")} ${k.label}`)
			.join(", "),
		named,
 ...
 *
 * Ordered by the lab run, which straightens the lab dots into a curve and
 * leaves the field dots scattered across them. Sorted the other way the picture
 * is far duller — the field p75s are tightly grouped, so the curve is nearly a
 * straight line and the scatter says the same thing with less of it visible.
 */
function buildLabFieldPairs(entries) {
	...
	const rowH = 4;
	/*
	 * The scale sits above the plot, not below it. One row per site makes this
	 * chart taller than any viewport, so an axis at the foot of it is off screen
	 * for the entire read — and the threshold labels were already up here, which
	 * left the numbers describing the same axis split across two ends of it.
	 */
	const box = { w: 1000, left: 86, right: 14, top: 62, bottom: 16 };
	...
	const pairs = rows.map((r, i) => {
		...
		const band = r.lab <= good ? "good" : r.lab <= poor ? "average" : "poor";
		return {
			y,
			labX: sx(r.lab),
			fieldX: sx(r.field),
			// Only the lab end is colored, being the end the rows are ordered by. A
			// second colored dot would leave the eye reading the verdict off
			// whichever end happens to sit further right.
			band,
			/*
			 * Named only if the lab run was a good paint, as on every other chart
	...
	return {
		...
		axis: {
			// One baseline for every number on the scale, thresholds included.
			tickY: box.top - 14,
			xLabel: { x: box.left + plot.w / 2, y: box.top - 36 },
		},
		count: rows.length,
		labeled: rows.filter((r) => r.lab <= good).length,
...
export async function buildReport({
	...
	const context = { resultsDir, cruxEnabled, perfectDays: new Map() };
	...
	/*
	 * The three categories that are companies rather than projects, for the
	 * employees chart.
	 *
	 * Not the whole corpus: "how many people work here" is a question with an
	 * answer for Microsoft and no answer at all for a static-site generator or
	 * somebody's blog, and a chart drawn over the rest would be measuring who
	 * happens to be incorporated.
	 */
	const COMPANY_GROUPS = ["big-tech", "hosts", "builders"];
	const companyIndex = indexCompanies(companies);
	const companyEntries = chartEntries.filter((e) =>
		(e.groups || [e.group]).some((g) => COMPANY_GROUPS.includes(g)),
	);
	/*
	 * Both URLs, because a site that has moved is keyed here under the address it
	 * was configured with and reported under the one it now answers on. Cloudflare
	 * Pages moved to cloudflare.com/products/pages and dropped out of the chart
	 * until this looked at both.
	 */
	const companyOf = (e) =>
		companyIndex.byUrl.get(normalizeUrl(e.url)) ??
		companyIndex.byUrl.get(normalizeUrl(e.configuredUrl ?? e.url)) ??
		null;
	const companyRows = canonicalPerCompany(companyEntries, companyOf);
	...
	const groups = Object.entries(config.groups).map(([id, group]) => {
		...
		return {
			...
			// Why, in the category's own words — the two that opt out do so for
			// unrelated reasons, so the sentence cannot live in the template.
			excludeFromPerfectScoresNote: group.excludeFromPerfectScoresNote ?? null,
			// Whether /compare/ may offer this category unprompted — its Shuffle
			// button and the category menu beside it, which read one list so the menu
			// cannot name something the pool refuses. On unless a category opts out;
			// typing a site's name or linking to it is never affected.
			compareEligible: group.compareEligible !== false,
			...
			// Read by /open-to-work/ to name where the tags come from.
			showHiringStatus: group.showHiringStatus === true,
			entries: sortByLeaderboard(groupEntries, compare),
			// The panel's contents, decided here because the ordering cannot be
			// expressed in a template: Nunjucks can sort neither on a computed
			// value nor on a nested attribute.
			//
			// Improvements only. The panel exists to point at sites worth looking
			// at, and a site whose score fell is not one of them — publishing a
			// leaderboard of the week's worst regressions puts a name and a number
			// on somebody's bad week, which is not what this page is for.
			//
			...
			topMovers: (group.movers === false ? [] : groupEntries)
				.filter(
					(e) =>
						e.trends?.performance?.significant &&
						e.trends.performance.vsPrevious &&
						e.trends.performance.vsPrevious.better === true,
				)
				// By percentage, not points: a 6-point rise from 30 is a bigger
				// story than the same rise from 90, and `pct` already says so.
				.sort((a, b) => b.trends.performance.vsPrevious.pct - a.trends.performance.vsPrevious.pct)
				.slice(0, 4)
				.map((e) => ({
					slug: e.slug,
					url: e.url,
	...
	// Same currency as the home page's own category list, so a chip only shows
	// up in a place that also names the category it points to.
	const homeListed = new Set(groups.filter((g) => g.showOnHomePageCategoryList).map((g) => g.id));
	for (let entry of entries) {
		entry.categories = (entry.groups || [entry.group])
			.map((id, i) => ({ id, name: entry.groupNames?.[i] ?? id, size: groupSize.get(id) ?? 0 }))
			.sort((a, b) => b.size - a.size || a.name.localeCompare(b.name));
		/*
		 * The subset of `categories` fit for the home page's results table.
		 *
		 * That table sits right below the category list the reader just read, so a
		 * chip naming a category absent from it points somewhere the reader has no
		 * way to have heard of yet — AI and 11ty Emeritus are both hidden from the
		 * list on purpose, one because it is early and one because it is a record
		 * of departures, and a chip for either reopens the question the list
		 * answered.
		 *
		 * `entry.categories` itself is untouched: site.njk's Ranks line means to
		 * name every category a site is actually ranked in, home-page-listed or
		 * not, and group.njk's chips are sized within one category rather than
		 * needing this distinction at all.
		 */
		entry.homeCategories = entry.categories.filter((cat) => homeListed.has(cat.id));
	}
	...
	return {
		...
		metrics: { SCORES, LAB_METRICS, FIELD_METRICS, COMBINED_METRICS, WEIGHT_METRICS, HEALTH_METRICS },
		...
		thirdPartyScatter: buildScatter(chartEntries, {
			xOf: (e) => e.latest?.lab?.scores?.performance,
			yOf: (e) => e.latest?.lab?.weight?.byType?.["third-party"]?.bytes,
			unit: "score",
			unitY: "bytes",
			xScale: "linear",
			xDomain: [0, 100],
			// The score carries the colors and the threshold lines, and it is now
			// the horizontal axis, so both follow it across.
			bandOn: "x",
			bandsAt: { good: 90, poor: 50, higherIsBetter: true },
			// Nine hundred of these sites load no third-party bytes at all. On the
			// vertical axis that zero needs the row rather than the column.
			zeroRow: true,
		}),
		...
		/*
		 * How much document there is, against what Lighthouse made of the page.
		 *
		 * Elements rather than bytes along the bottom, which is the reason this
		 * chart is not the page-weight one again: a large DOM is paid for in style
		 * and layout work on every frame rather than in download, and the two come
		 * apart — the widest node counts here sit on pages of unremarkable size.
		 *
		 * Log scale, because the corpus runs from two elements to seventy-eight
		 * thousand and the interesting half of it is under five hundred.
		 */
		domScatter: buildScatter(chartEntries, {
			xOf: (e) => e.latest?.lab?.scores?.performance,
			yOf: (e) => e.latest?.lab?.dom?.elements,
			unit: "score",
			unitY: "count",
			xScale: "linear",
			xDomain: [0, 100],
		...
		/*
		 * Company headcount against paint time, for the three categories that are
		 * companies: Big Tech, Web Hosts, Website Builders.
		 *
		 * The one chart on this page with no mechanism behind it. Page weight
		 * causes paint time and TTFB is a component of it; headcount touches a
		 * marketing homepage's critical path not at all.
		 *
		 * Which is what makes the result worth drawing: the drift is there anyway,
		 * and it runs the wrong way. Sites belonging to companies of ten thousand
		 * people or more take roughly two and a half times as long to paint as the
		 * rest of this set. Correlation and not cause — bigger companies ship more tag
		 * managers, more consent tooling and more marketing stack, and that is
		 * what the axis is picking up — but the direction is not the one anyone
		 * would defend out loud.
		 *
		 * Log scale for the same reason as the others, and more so: 1 employee to
		 * 1,556,000 is six decades, the widest axis on the site.
		 */
		employeeScatter: buildScatter(companyRows, {
			xOf: (e) => companyOf(e)?.employees ?? null,
			unit: "count",
			nameAll: true,
			nameOf: (e) => {
				const company = companyOf(e);
				// Named for the parent where the site is not the one the company is
				// known by, which happens only when the flagship had no measurement and
				// a subsidiary property stood in for it.
				const owner = company && normalizeUrl(company.urls[0]) !== normalizeUrl(e.url);
				return owner ? `${e.displayUrl} (${company.name})` : e.displayUrl;
			},
		}),
		// The companies measured but absent from the plot, having no published
		// headcount. By company rather than by site, to match the dots. The page
		// shows the count; the names are here for when it wants them.
		employeesUnknown: [
			...new Set(companyRows.filter((e) => !companyOf(e)?.employees).map((e) => companyOf(e).name)),
		].sort(),
		...
		siteOfTheDay: siteOfTheDay(entries, now, context.perfectDays),
 ...
 *
 * A category opts out with `excludeFromPerfectScores: true`, and that only
 * reaches a site whose categories *all* opt out — in practice, a site listed
 * nowhere else. Belonging to one ordinary category is enough to earn a place on
 * the board, whatever else the site is also filed under.
 *
 * The opt-out is a statement about a category, not about the sites in it: that
 * these pages are not what the board is for. A site that also stands on a
 * public list is not in that position, and excluding it would be judging it by
 * the company it keeps rather than by anything it is.
 *
 * `every` rather than a length check, so two opted-out categories on one site
 * still exclude it — the site is then in exactly the position the flag
 * describes, twice over.
 *
...
function flagPerfectScoreExclusions(entries, config) {
	...
	for (let entry of entries) {
		entry.excludedFromPerfectScores = (entry.groups || [entry.group]).every((id) => excluded.has(id));
	}
}
...
/** Keep a site's hiring tag only when one of its categories sets `showHiringStatus: true`. */
function flagHiring(entries, config) {
	const shown = new Set(
		Object.entries(config.groups)
			.filter(([, group]) => group.showHiringStatus === true)
			.map(([id]) => id),
	);

	for (let entry of entries) {
		if (!(entry.groups || [entry.group]).some((id) => shown.has(id))) entry.hiring = null;
	}
}
...
function buildEntry(site, store, config, now, aliases, context) {
	...
	// Predecessors that are an actual change of address. A landing path this
	// site's own root redirects to is in the history because measurements were
	// once filed under it, not because the site ever lived there — saying it
	// "moved from" its own homepage would read backwards.
	const moveFrom = predecessors.filter((u) => !isLandingRedirect(site.url, u));
	...
	// The newest record whether or not it succeeded, for the notices that
	// describe the site's current state rather than its last good numbers.
	const newest = latestPoint?.error ? latestAcross(store, historyUrls) : latest;

	const generator = reDetectGenerator(latest?.axe?.generator);

	// The URL the site redirected us to, when that redirect is not a move: a
	// trailing slash, or a root origin pointing at the landing path it serves
	// there. A real move is handled by the redirect machinery instead; these are
	// the cases that machinery deliberately ignores, and this is where the form
	// the site actually serves is recorded so the redirect is not invisible.
	//
	// Which of the two it is decides what the notice can honestly say, so it is
	// recorded rather than re-derived from the URLs in a template: a root origin
	// pointing at `/us-en` is not a trailing slash, and describing it as one is
	// visibly wrong on the page.
	const finalUrl = latest?.lab?.finalUrl ?? null;
	const canonicalKind =
		!finalUrl || finalUrl === site.url
			? null
			: normalizeUrl(finalUrl) === normalizeUrl(site.url)
				? "normalized"
				: isLandingRedirect(site.url, finalUrl)
					? "landing"
					: null;
	const canonicalUrl = canonicalKind ? finalUrl : null;
	...
	const redirectTo =
		storedTo && normalizeUrl(storedTo) !== normalizeUrl(site.url) && !isLandingRedirect(site.url, storedTo)
			? storedTo
			: null;
	...
	const interstitial = newest?.axe?.interstitial ?? null;
	const filmstrip = olderThanCheck(
		readFilmstrip(context.resultsDir, site.hash, latest?.lab?.environment?.throttlingMethod ?? null),
		interstitial,
		newest
	);
	const ownScreenshot = withoutChallengedNoJs(readOwnScreenshot(context.resultsDir, site.hash), interstitial);
	...
	const entry = {
		...
		movedFrom: moveFrom.length ? moveFrom[moveFrom.length - 1] : null,
		...
		moveAt: moveFrom.length ? findMoveBoundary(series) : null,
		redirectTo,
		...
		metricsTable: buildMetricsTable(latest, cwv),
		...
		axe: latest?.axe ?? null,
		/*
		 * The bot check currently standing between us and this site, if there is
		 * one. Read off the newest record rather than the newest successful one:
		 * a challenged run is now stored as a failure, so the record that knows
		 * about the check is exactly the one `latest` skips.
		 */
		interstitial,
		...
		// What else the page runs. Null, not empty, for a record written before
		// the probe was kept: that is unknown rather than nothing.
		tools: latest?.axe?.probe ? detectTools(latest.axe.probe, latest.axe.headers ?? {}, generator) : null,
		// Cleared by `flagHiring` unless one of the site's categories opts in.
		hiring: hiringStatus(latest?.axe?.probe?.hiring),
	...
	// Perfect, and it was not on the run before — the same transition the stat
	// bar's "Newly perfect" card lists, carried onto the entry so the
	// leaderboard can mark the row without rebuilding that list per template.
	//
	// No window on it, unlike the card: this describes the measurement the row
	// is showing, so it holds until the site is measured again. On a slow
	// cadence that is longer than the card's 48 hours, and the pill outliving
	// the card entry is the intended behavior rather than a drift between them.
	entry.newlyPerfect = entry.perfect && becamePerfectAt(series, entry.cwvFailures) !== null;
...
/*
 * The newest record of any kind, failures included.
 *
 * `latestSuccessAcross` deliberately walks past a failed run to the last good
 * one, which is what the numbers on a page should come from. It is the wrong
 * source for what is happening to the site *now* — a site being served a bot
 * check has no successful record to carry that fact, and reading the state off
 * the last good measurement would report a challenged site as fine.
 */
function latestAcross(store, urls) {
	for (let i = urls.length - 1; i >= 0; i--) {
		const record = store.latest(urls[i]);
...
/*
 * The refusals the measurement step now applies, for records written before it
 * did.
 *
 * Measurements are kept forever and never rewritten, so without this the three
 * failed DOM snapshots already on disk go on being published as two-element
 * pages, and devin.ai and exyon.ee go on being ranked on a checkpoint's
 * accessibility results. Both are fixed at the point of measurement now; this is
 * the same judgment applied on the way out, so the fix does not have to wait for
 * every site to come round again.
 *
 * Copied rather than edited in place: the store hands out the record it has
 * cached, and the raw file is the evidence of what was actually measured. Only
 * the copy this report renders from forgets, and only where a guard trips, so
 * every other record is passed straight through untouched.
 */
function normalizeRecord(record) {
	let out = record;

	if (domSnapshotFailed(out?.lab?.dom)) {
		out = {
			...out,
			lab: { ...out.lab, dom: { elements: null, depth: null, maxChildren: null } },
		};
	}

	// A challenged accessibility pass describes the waiting room, not the site.
	// The lab numbers beside it are a real measurement and are left alone — see
	// `challengedAxe` in lib/stack.js for why the host survives and the
	// findings do not.
	if (out?.axe?.interstitial) {
		out = { ...out, axe: challengedAxe(out.axe) };
	}
...
function coreWebVitals(latest, cruxEnabled, fieldHistory) {
	...
	// Lab has no INP, so this is a two-of-three approximation. The site page
	// never states it as a verdict — it marks the rows it would cover `CWV
	// (estimate)` and leaves the numbers in the Lab column, which is the column
...
/**
 * Bands for the two lab timings CrUX has no threshold for, so every row in the
 * Lab column can carry a rating rather than most of them. Lighthouse's own
 * boundaries, which is what its report colors these two by.
 */
const LAB_ONLY_BANDS = {
	tbt: { good: 200, poor: 600 },
	si: { good: 3400, poor: 5800 },
};

/**
 * A lab timing's rating, on Google's published thresholds wherever there is
 * one. Same bands the field half is graded on, deliberately: the two columns
 * sit side by side now, and grading them differently would make a lab number
 * and a field number of the same size wear different colors.
 */
function labRating(key, value) {
	const t = CWV_THRESHOLDS[key] ?? LAB_ONLY_BANDS[key];
	if (!t) return null;
	return band(value, t.good, t.poor);
}

/**
 * The site page's metrics table, one row per metric with both halves filled.
 *
 * Takes the Core Web Vitals assessment only to read the field data out of it:
 * for a site measured before the CrUX key existed, the backfilled week the
 * assessment fell back to is the only real-user data there is.
 */
function buildMetricsTable(latest, cwv) {
	if (!latest) return [];

	const timings = latest.lab?.timings ?? null;
	const field = latest.field?.metrics ?? null;

	// Keyed by the row each one belongs to, which is the metric's own key.
	const parts = new Map((cwv?.parts ?? []).map((part) => [part.key, part]));
	const fieldSourced = cwv?.source === "field" || cwv?.source === "field-history";

	return COMBINED_METRICS.map((row) => {
		const labValue = row.lab ? (timings?.[row.lab] ?? null) : null;
		const part = parts.get(row.key) ?? null;

		// This measurement's own CrUX record first. Falling back to the week the
		// assessment used keeps the numbers for a site measured before the API
		// key existed, whose field data lives only in the backfilled history —
		// without it, folding the panel in here would have quietly dropped them.
		let f = row.field ? field?.[row.field] : null;
		let fieldSource = f ? "field" : null;
		if (row.field && !f && fieldSourced && part) {
			f = { p75: part.value, rating: part.rating, distribution: part.distribution, scope: part.scope };
			fieldSource = cwv.source;
		}
		...
		return {
			...
			note: row.note ?? null,
			unit: row.unit,
			coreWebVital: Boolean(row.coreWebVital),
			// Null means the metric does not exist on this side at all — see
			// COMBINED_METRICS. A present half with a null value is a measurement
			// that is simply missing.
			lab: row.lab ? { value: labValue, rating: labRating(row.lab, labValue) } : null,
			field: row.field
				? {
						value: f?.p75 ?? null,
						rating: f?.rating ?? null,
						distribution: f?.distribution ?? null,
						// Borrowed from the origin because this page had too few
						// samples, and labeled on the page as such.
						scope: f?.scope ?? null,
						// Which 28 days: this measurement's, or the backfilled week.
						source: fieldSource,
					}
				: null,
			// How the lab half's history is keyed in `trends`, so the template does
			// not have to know that the two key spaces happen to agree.
			trendKey: row.lab,
		};
	});
}
...
/*
 * A filmstrip taken on the run that met the bot check, which can be of the check itself.
 *
 * Runs since then leave the previous strip on disk instead, so this only
 * catches strips written before they did (www.producthunt.com's is Cloudflare's).
 */
function olderThanCheck(filmstrip, interstitial, newest) {
	if (!interstitial || !filmstrip) return filmstrip;
	return filmstrip.timestamp === newest?.timestamp ? null : filmstrip;
}

/*
 * A no-JS pair behind a bot check, unless it provably came from an unchallenged pass.
 *
 * Challenged passes have not stored captures since before `capturedAt` was
 * written, so a stamped pair is of the site and an unstamped one may be of the check.
 */
export function withoutChallengedNoJs(screenshot, interstitial) {
	if (!interstitial || !screenshot?.noJs || screenshot.capturedAt) return screenshot;
	return { ...screenshot, noJs: null, difference: null, noJsText: null };
}
...
function buildClientRenderedByGenerator(entries) {
	...
	/*
	 * Only the generators that actually flagged something get a row.
	 *
	 * Twenty-three of the twenty-nine here flagged nothing, and nineteen of those
	 * were judged on five sites or fewer. Drawn, they are two dozen rows of a dot
	 * sitting on a dot at zero — a chart whose dominant visual feature is the
	 * absence of a finding, and which is four screens tall on a phone before it
	 * reaches the six rows worth looking at.
	 *
	 * They are not deleted, they are counted: the page prints how many generators
	 * came up clean and across how many sites, which is the same fact stated in
	 * the form it deserves. A row saying "Hugo: 0 of 2" is not evidence about
	 * Hugo, and drawing it as one overstates what two sites can say.
	 */
	const drawn = rows.filter((r) => r.flagged > 0);
	const clean = rows.filter((r) => r.flagged === 0);
	...
	return {
		...
		pairs: clientRenderedPairs(drawn),
		// The rows not drawn, as the sentence that replaces them.
		clean: { generators: clean.length, judged: clean.reduce((n, r) => n + r.judged, 0) },
		judged: judged.length,
		flagged: judged.filter((e) => e.clientRendered).length,
		tools: rows.filter((r) => r.detected).length,
		withFlagged: drawn.length,
	};
}

/*
 * The flagged generators as a dumbbell, in the same language as Lab vs Field.
 *
 * One row per generator, a dot for how many of its sites were judged and a dot
 * for how many of them drew nothing, joined by a line. The length of that line
 * is the sites that rendered — so the chart shows the finding and the sample it
 * came out of in one mark, which is the whole difficulty with this data. A row
 * of one site cannot draw a long line no matter how bad its one site was, and
 * "PBC on Nunjucks, 1 of 1" cannot be mistaken for a verdict on a tool.
 *
 * Counts, not rates. On a percentage axis that row is 100% and sits at the far
 * end, above Next.js on thirteen sites — a chart that answers the question by
 * ranking its least-supported row first.
 *
 * Linear, not log. The axis is a count of sites running from zero to about
 * sixty, zero is a value every row needs to be able to sit at, and no reading
 * here turns on the difference between one site and two being drawn the same
 * width as the difference between ten and twenty.
 */
function clientRenderedPairs(rows) {
	if (!rows.length) return null;

	const hi = Math.max(...rows.map((r) => r.judged));
	// Padded at the right so the widest row's dot sits inside the frame, and the
	// axis always starts at a true zero — the left end of a row is a real count.
	const x1 = hi * 1.04;

	const rowH = 40;
	// The same left margin the strip plots use, holding the same kind of thing:
	// a generator's name and the size of its sample.
	const box = { w: 1000, left: 184, right: 14, top: 16, bottom: 70 };
	box.h = box.top + rows.length * rowH + box.bottom;
	const plot = { w: box.w - box.left - box.right, h: rows.length * rowH };

	const sx = (n) => Math.round(box.left + (n / x1) * plot.w);
	...
	const pairs = rows.map((r, i) => {
		...
		return {
			...r,
			y,
			flaggedX: sx(r.flagged),
			judgedX: sx(r.judged),
			// The share, as words rather than as a coordinate. It is the number a
			// reader wants and the number that must not be plotted: printed beside
			// a row whose sample is right there, it cannot be read off the axis and
			// compared with a row of one site.
			rate: `${r.flagged} of ${r.judged}`,
		};
	});

	/*
	 * Whole sites, at a step that gives five or six gridlines.
	 *
	 * Ticks land on integers because the axis counts sites and a gridline at 7.5
	 * of them is not a quantity. The step is chosen from the round numbers rather
	 * than by dividing the range, for the same reason.
	 */
	const step = [1, 2, 5, 10, 20, 25, 50, 100, 200].find((v) => x1 / v <= 7) ?? 500;
	const xTicks = [];
	for (let v = 0; v <= x1; v += step) xTicks.push({ x: sx(v), label: String(v) });
...
export function clientRendered(screenshot) {
...
/** Null when nothing was recorded, so "unknown" never reads as "unthrottled". */
function throttledFrom(method) {
	if (typeof method !== "string" || !method) return null;
	return method !== "simulate";
}

/**
 * Moves the first rendered frame back to FCP when content painted before Lighthouse took it.
 *
 * The first rendered frame is the first one that differs from the pre-paint frames leading the
 * strip, or the very first frame when none were taken before FCP. Otherwise the FCP and LCP flags
 * sit ahead of any picture: remix.run's first frame lands at 5.6s after a 1.4s FCP, and
 * palantir.com's blank frames give way at 12.16s after an 11.56s FCP. A settle on that frame
 * moves with it.
 */
export function alignFirstFrame(frames, markers, settledAt) {
	const fcp = markers?.fcp;
	if (typeof fcp !== "number" || !frames.length) return { frames, settledAt };

	let index = 0;
	if (frames[0].timing < fcp) {
		index = frames.findIndex((f) => f.src !== frames[0].src);
		if (index === -1 || frames[index - 1].timing >= fcp) return { frames, settledAt };
	}

	const taken = frames[index].timing;
	if (typeof taken !== "number" || fcp >= taken) return { frames, settledAt };

	return {
		frames: frames.map((f, i) => (i === index ? { ...f, timing: fcp } : f)),
		settledAt: settledAt === taken ? fcp : settledAt,
	};
}

/** Closer than this to a neighbor, a frame at LCP would overlap it: Lighthouse's finest usual interval, and the site player's step. */
const LCP_FRAME_CLEARANCE = 375;

/**
 * `alignFirstFrame`, plus a frame at LCP when LCP falls in a gap between frames, and a settle no earlier than LCP.
 *
 * The LCP frame shows the first frame taken after LCP, the earliest picture with the LCP element in it.
 * bolt.new's first painted frame lands at 16.86s and moves to its 14.08s FCP, which left its 16.07s LCP in
 * an empty gap and the page settled before it.
 */
export function alignFrames(frames, markers, settledAt) {
	const aligned = alignFirstFrame(frames, markers, settledAt);
	const lcp = markers?.lcp;
	if (typeof lcp !== "number" || (typeof markers.fcp === "number" && lcp < markers.fcp)) return aligned;

	let out = aligned.frames;
	const after = frames.find((f) => f.timing > lcp);
	const next = out.findIndex((f) => f.timing >= lcp);
	if (
		after &&
		next > 0 &&
		out[next].timing - lcp >= LCP_FRAME_CLEARANCE &&
		lcp - out[next - 1].timing >= LCP_FRAME_CLEARANCE
	) {
		out = [...out.slice(0, next), { timing: lcp, src: after.src }, ...out.slice(next)];
	}

	let settled = aligned.settledAt;
	if (typeof settled === "number" && settled < lcp) {
		settled = out.find((f) => f.timing >= lcp)?.timing ?? settled;
	}
	return { frames: out, settledAt: settled };
}

function readFilmstrip(resultsDir, hash, fallbackThrottling = null) {
	...
	const markers = manifest.markers && typeof manifest.markers === "object" ? manifest.markers : null;
	const aligned = alignFrames(
		frames.map((f) => ({ timing: f.timing ?? null, src: src(f.file) })),
		markers,
		typeof manifest.settledAt === "number" ? manifest.settledAt : null
	);
	...
	return {
		...
		// When the page stopped changing, judged on the pixels — see settleTime
		// in lib/metrics.js. Null for any strip stored before that was measured,
		// which the pages fall back from by comparing frame filenames.
		settledAt: aligned.settledAt,
		/*
		 * Whether these frames are of a throttled load.
		 *
		 * "simulate" is Lighthouse's default and does not throttle the browser: it
		 * loads the page at full speed and models the metrics afterwards. So a
		 * simulated strip is a fast load, and a devtools-throttled one is a slow
		 * connection — the two are not comparable, whatever the numbers beside
		 * them say.
		 *
		 * The manifest is authoritative; the fallback is the run the strip was
		 * written from, for strips stored before the field existed.
		 */
		throttled: throttledFrom(manifest.throttlingMethod ?? fallbackThrottling),
		markers,
...
export function ringBands(entry) {
	return [
...
function reDetectGenerator(stored) {
	...
	// Found on the page by a rule that no longer counts as a generator, like htmx or Svelte.
	if (stored.source === "dom" && !generatorById(stored.id)) return null;
...
function buildStacks(entries) {
	// `pick` returns a list so a site can count toward several tools; null skips
	// the site entirely rather than counting it as unknown.
	const tally = (pick, iconOf) => {
		...
		for (let entry of entries) {
			...
			if (!picked) continue;
			const detected = picked.filter((d) => d?.name);
			if (!detected.length) {
				unknown++;
				continue;
			}
			...
			for (let d of detected) {
				const existing = counts.get(d.name);
				if (existing) existing.count++;
				else counts.set(d.name, { name: d.name, icon: iconOf(d), count: 1 });
			}
		}
	...
	return {
		...
		// Counts sites, so the rows add up to more than the heading.
		tools: tally((e) => e.tools, (d) => d.icon ?? null),
...
/**
 * One entry per company: the canonical site, not every site it owns.
 *
 * Five companies here own more than one measured site — Amazon holds both
 * amazon.com and aws.amazon.com/amplify, Microsoft three, Alphabet three — and
 * plotting all of them stacked several dots in a single column. That gave the
 * larger companies more weight in a chart where each is meant to count once, and
 * read as though headcount varied within a company when it is the same figure
 * repeated.
 *
 * The canonical site is `urls[0]` in config/companies.js. A company whose
 * flagship has no usable measurement falls back down its own list rather than
 * dropping out of the chart, which is why this ranks the candidates instead of
 * just filtering for the first URL.
 */
function canonicalPerCompany(entries, companyOf) {
	...
	const position = (company, entry) => {
		const at = company.urls.findIndex(
			(u) =>
				normalizeUrl(u) === normalizeUrl(entry.url) ||
				normalizeUrl(u) === normalizeUrl(entry.configuredUrl ?? entry.url),
		);
		return at === -1 ? company.urls.length : at;
	};

	const rows = [];
	for (const [company, owned] of byCompany) {
		const measured = owned.filter((e) => typeof e.latest?.lab?.timings?.lcp === "number" && e.latest.lab.timings.lcp > 0);
		const pool = measured.length ? measured : owned;
		rows.push([...pool].sort((a, b) => position(company, a) - position(company, b))[0]);
	}
	return rows;
}
 ...
 */
function siteOfTheDay(entries, now, perfectDays) {
	...
	// Earlier days replay against each site's last verdict as of that day.
	const candidates = entries
		.filter((e) => !e.excludedFromPerfectScores && !e.interstitial && perfectDays.has(e.url))
		.map((entry) => ({ entry, days: perfectDays.get(entry.url) }));
	const poolFor = (d) => {
		if (d === day) return featured;
		return candidates
			.filter(({ days }) => {
				let perfect = false;
				for (let p of days) if (p.day <= d) perfect = p.perfect;
				return perfect;
			})
			.map(({ entry }) => entry);
	};

	const chosen = selectSiteOfTheDay(poolFor, day);
...
/**
 * How many days a daily series needs before it is drawn at all.
 *
 * A handful of days is not a trend: the fleet is measured on a rolling
 * schedule, so each day's figure is drawn from whichever slice happened to come
 * up, and over a short window that composition moves the line far more than the
 * sites do. Given enough days the sampling evens out and the shape means
 * something.
 */
...
function buildPerfectHistory(perfectDays, entries) {
	...
	if (ordered.length < MIN_HISTORY_DAYS) return [];
...
/**
 * When a perfect site crossed over, or null if its latest run was not the one.
 *
 * The transition alone, with no window on it: the run before was not perfect
 * and this one is. The stat bar's card puts NEWLY_PERFECT_WINDOW_HOURS around
 * the answer because it is reporting news; the leaderboard's pill does not,
 * because it is describing the run in the row beside it — which stands until
 * the site is measured again, however long that takes.
 *
 * Only half the test: the caller decides whether the site is perfect *now*, on
 * whichever `perfect` predicate it uses. This answers the other half, on
 * `pointIsPerfect` — the stored point carries the four scores and the axe count,
 * which is what that predicate needs. Failed runs are skipped when looking back:
 * a site that errored once has not "become" perfect by being measured again.
 *
 * Shared so the card and the pill cannot disagree about which run turned a site.
 */
function becamePerfectAt(points, cwvFailures) {
	const clean = points.filter((p) => !p.error);
	if (clean.length < 2) return null;
	if (pointIsPerfect(clean[clean.length - 2], cwvFailures)) return null;
...
function buildStats(entries, { featuredOnly = false, now = Date.now() } = {}) {
	...
	const newlyPerfect = measured
		.filter((e) => perfect(e))
		.map((e) => {
			const timestamp = becamePerfectAt(e.history ?? [], e.cwvFailures);
	...
	return {
		...
		newlyPerfect,
		// So the card can name its own window without hardcoding the number.
		newlyPerfectWindowHours: NEWLY_PERFECT_WINDOW_HOURS,
		/*
