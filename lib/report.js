...
function buildScatter(entries, {
	...
	// carries LCP so the colours and the 2.5 s / 4 s lines follow it.
	yOf = (e) => e.latest?.lab?.timings?.lcp,
	unit = "bytes",
	unitY = "ms",
	bandOn = "y",
	// Which Core Web Vital the colours and threshold lines come from. Not always
	...
	// Banded by the same thresholds the rings and tables use, so a colour means
	...
	return {
		...
		axis: {
			...
			// Rotated about its own centre, so it reads bottom-to-top alongside
			// the scale it names.
		...
		zero: zoneW
			...
			: null,
		/*
		 * The two lines that make the colours a scale rather than a palette.
...
function buildLabFieldPairs(entries) {
	...
	const pairs = rows.map((r, i) => {
		...
		return {
			...
			labX: sx(r.lab),
			fieldX: sx(r.field),
			// Coloured by what real users got, which is the half of the pair that is
	...
	return {
		 ...
		 * target taller than that overlaps its neighbours and the hover lands on
		...
		count: rows.length,
		labelled: rows.filter((r) => r.field <= good).length,
 ...
 *
 * Labelled with both names. The current one leads, and the old one is kept in
 ...
 * corpus. Eleventy is eighty per cent of the sites that report a generator, and
	...
	return Object.entries(byType)
		...
		.map(([key, part]) => {
			...
			return {
				 ...
				 * one. Anything present but below a tenth of a per cent says so.
				 */
...
export async function buildReport({
	...
	return {
		...
		lcpScatter: buildScatter(entries, {
		...
		// meaningful zero.
		jsScatter: buildScatter(entries, {
		...
		fontClsScatter: buildScatter(entries, {
		...
		labFieldPairs: buildLabFieldPairs(entries),
		 ...
		 * colours: ninety and above is green, and the axis stops at a hundred
		...
		jsByGenerator: buildStrip(entries, {
			keyOf: generatorKey,
		...
		ttfbByHost: buildStrip(entries, {
			keyOf: (e) => e.host,
			valueOf: (e) => e.latest?.lab?.timings?.ttfb,
			unit: "ms",
			label: "host",
		}),
		thirdPartyScatter: buildScatter(entries, {
		...
		ttfbScatter: buildScatter(entries, {
...
function buildStats(entries, { featuredOnly = false } = {}) {
	...
	return {
		 ...
		 *
		 * The colour means the same on both even though the axes run opposite ways
