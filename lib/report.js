...
/*
 * How empty the no-JS capture has to be, in percent of scanlines, before a site
 * is called client-rendered.
 *
 * Read off the corpus and drawn through a gap rather than a crowd: the
 * server-rendered pages top out at 48.6% (nuxt.com, which is mostly whitespace
 * and still renders everything) and the client-rendered ones start at 74.1%
 * (mistral.ai, a header bar over an empty body). Anywhere in between gives the
 * same answer on this data.
 *
 * This replaced a two-part rule — few distinct colors, plus a floor on the pixel
 * difference against the rendered capture — that missed two sites the eye
 * catches immediately. x.ai is blank behind a hamburger button but scores only
 * 21.5% difference, because the page is white with sparse dark text either way.
 * mistral.ai draws a colorful logo in its header and scores 274 colors. Neither
 * survives contact with a real corpus; empty rows do.
 */
const CLIENT_RENDERED_MIN_EMPTY = 60;

function clientRendered(screenshot) {
	if (!screenshot?.noJs) return null;
	if (typeof screenshot.noJsEmpty !== "number") return null;

	return screenshot.noJsEmpty >= CLIENT_RENDERED_MIN_EMPTY;
}
