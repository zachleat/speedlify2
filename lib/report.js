 ...
 *
 * The Core Web Vitals assessment is folded in rather than given a panel of its
 * own, because every number it reports is already a cell in this table — the
 * field p75 for a site CrUX knows, the lab timing for one it does not. What the
 * panel added was the verdict and where it came from, and those ride on the row
 * now.
 */
function buildMetricsTable(latest, cwv) {
	...
	// Keyed by the row it belongs to, which is the metric's own key everywhere
	// except the lab approximation's TBT — that part is keyed `tbt` and declares
	// the INP it stands in for, so it lands on the TBT row and says why.
	...
	return COMBINED_METRICS.map((row) => {
		...
		return {
			...
			field: row.field
				...
				: null,
			// The row's part in the Core Web Vitals verdict, or null if it has
			// none. `approximated` is the one the page must not stay quiet about:
			// the number is a Lighthouse timing standing in for an assessment of
			// real users, and it reads identically to the real thing otherwise.
			cwv: part
				? {
						rating: part.rating ?? null,
						value: part.value ?? null,
						source: cwv.source,
						approximated: cwv.source === "lab",
						// Set only on the lab approximation's TBT, which is graded in
						// place of the INP no lab run can measure.
						proxyFor: part.proxyFor ?? null,
					}
				: null,
