 ...
 *
 * Enough to colour a dot and nothing more: a row of these is a glance, and the
...
/* Short on purpose: this sits under a list of twelve and is a footnote to it,
   not a second list of the same weight. */
const NEWLY_PERFECT = 5;
...
function buildStats(entries, { featuredOnly = false } = {}) {
	...
	return {
		...
		recentlyMeasured: measured
			...
			/*
			 * Shortened by however many newly-perfect rows sit under it.
			 *
			 * The two lists are one card, and the card's height is the row's height —
			 * so a day when five sites turn perfect should not push every other card
			 * in the bar five rows taller. The pair together is always the same length.
			 */
			.slice(0, Math.max(1, RECENTLY_MEASURED - newlyPerfect.length))
			.map((e) => {
		...
		cwvVerdicts: (() => {
			...
			return {
				...
				bars: [
					...
					// colour at all and it fell back to the accent.
