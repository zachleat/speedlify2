...
function buildEntry(site, store, config, now, aliases, context) {
	...
	const filmstrip = readFilmstrip(context.resultsDir, site.hash, latest?.lab?.environment?.throttlingMethod ?? null);
	...
	const entry = {
		...
		clientRendered: clientRendered(ownScreenshot, newest?.axe?.interstitial ?? null),
		 ...
		 */
		interstitial: newest?.axe?.interstitial ?? null,
...
export function clientRendered(screenshot, interstitial = null) {
	/*
	 * A bot check is not a site, and neither capture is of one.
	 *
	 * Both halves of the test read as innocent on a wall: it has almost no text,
	 * and it looks the same with scripts and without, so the difference is zero
	 * and the page comes back "not client-rendered" — a verdict about
	 * tesla.com's Access Denied page rather than about tesla.com. The screenshot
	 * is already suppressed for exactly this reason; the verdict has to follow
	 * it, and unjudged is the honest answer.
	 */
...
function buildStacks(entries) {
	...
	return {
		 ...
		 *
		 * Implied tools are left out, for the reason a presumed generator is: this
		 * is a count of what was detected, and "React, 300" carried mostly by
		 * Next.js sites would be inference wearing a measurement's clothes. The
		 * site's own Stack panel states them.
		 */
		tools: tally((e) => e.tools && e.tools.filter((t) => !t.implied), (d) => d.icon ?? null),
