...
function readOwnScreenshot(resultsDir, hash) {
	...
	return {
		...
		noJsEmpty: noJs && typeof manifest.noJsEmpty === "number" ? manifest.noJsEmpty : null,
	};
}
 ...
 *
 * Emptiness — the share of scanlines holding a single flat color — answers "was
 * anything drawn". It is the only measure that survives ti.com's gray panel,
 * mistral.ai's colorful header over an empty body, and x.ai's white-on-white
 * page. But it cannot tell an empty page from a roomy one: cloudflare.com's
 * products page is 61% uniform rows and google.com 80%, and both render
 * everything server-side. A page with wide margins is mostly empty rows by
 * design.
 *
 * The difference against the rendered capture answers "did running the scripts
 * change it". Those roomy pages score 0–1%: JavaScript does nothing visible,
 * which is the whole point. On its own it is no good either — x.ai is white
 * behind sparse text either way and moves only 21.5% of its pixels on a total
 * render, which is why an earlier 25% floor missed it.
 *
 * Together they say what the eye says: nothing was there, and running the
 * scripts put something there.
 *
 * 20% is the lowest difference floor that still keeps x.ai, and what it
 * excludes sits at 17.4% (lit.dev) and below — the line falls in a gap.
 */
const CLIENT_RENDERED_MIN_EMPTY = 60;
const CLIENT_RENDERED_MIN_DIFFERENCE = 20;

function clientRendered(screenshot) {
	if (!screenshot?.noJs) return null;
	if (typeof screenshot.noJsEmpty !== "number" || typeof screenshot.difference !== "number") return null;

	return (
		screenshot.noJsEmpty >= CLIENT_RENDERED_MIN_EMPTY &&
