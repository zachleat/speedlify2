 ...
 * moves with it, but no earlier than an LCP painted before the frame: bolt.new's 16.86s frame
 * would otherwise settle at its 14.08s FCP, ahead of a 16.07s LCP.
 */
export function alignFirstFrame(frames, markers, settledAt) {
	...
	const floor = typeof lcp === "number" && lcp > fcp && lcp < taken ? lcp : fcp;

	return {
		frames: frames.map((f, i) => (i === index ? { ...f, timing: fcp } : f)),
		settledAt: settledAt === taken ? floor : settledAt,
	};
}
