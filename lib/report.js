...
function readOwnScreenshot(resultsDir, hash) {
	...
	return {
		...
		// Distinct colors in the no-JS capture, quantized. See `clientRendered`.
		noJsColors: noJs && typeof manifest.noJsColors === "number" ? manifest.noJsColors : null,
	};
}

/*
 * Thresholds for calling a site client-rendered, both read off the corpus.
 *
 * Color count splits it cleanly: of 116 sites with a no-JS capture, nine come
 * in at three colors or fewer and the next is at seventy. Nothing at all sits
 * between four and fifty, so the line is drawn in an empty gap rather than
 * through a crowd, and its exact value inside that gap changes nothing.
 *
 * The difference floor is there to catch a specific false positive: a capture
 * that is blank because the *page* failed, not because its content needed
 * scripts. meta.com is one flat color with and without JavaScript — a block
 * page, scoring 6% difference. Every genuinely script-dependent site in the
 * corpus scores above 43%. So a site only qualifies if JavaScript demonstrably
 * changed what was on screen.
 */
const CLIENT_RENDERED_MAX_COLORS = 16;
const CLIENT_RENDERED_MIN_DIFFERENCE = 25;

/**
 * Did this page need JavaScript to draw anything at all?
 *
 * Two conditions, because either alone is wrong. A high difference on its own
 * says only that scripts changed the view — nuxt.com scores 99.8% and renders
 * completely without them. A blank capture on its own can mean the load failed.
 * Together they say: there was nothing here, and running the scripts is what
 * put something here.
 *
 * Null rather than false when there is no no-JS capture to judge — most of the
 * corpus, which opts out of the second screenshot.
 */
/*
...
function clientRendered(screenshot) {
	...
	if (typeof screenshot.noJsColors !== "number" || typeof screenshot.difference !== "number") return null;

	return (
		screenshot.noJsColors <= CLIENT_RENDERED_MAX_COLORS &&
...
function buildStats(entries, { featuredOnly = false, now = Date.now() } = {}) {
	...
	return {
		...
		recentlyMeasured: measured
			 ...
			 *
			 * The two lists share one card, and the card's height is the row's
			 * height — so a day when a dozen sites turn perfect should not push
			 * every other card in the bar a dozen rows taller. Newly perfect has
			 * the stronger claim on the space: it reports a change somebody made,
			 * while this list reports the measurement loop doing its rounds. So it
			 * yields, and the template drops its heading entirely at zero rather
			 * than printing a label over an empty list.
			 */
			.slice(0, Math.max(0, RECENTLY_MEASURED - newlyPerfect.length))
			.map((e) => {
				const bands = ringBands(e);
