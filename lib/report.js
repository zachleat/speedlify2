...
function buildEntry(site, store, config, now, aliases, context) {
	...
	// Same window as the card, so a site wears the pill for exactly as long as
	// it sits in the list above.
	entry.newlyPerfect = entry.perfect && newlyPerfectAt(series, entry.cwvFailures, now) !== null;
...
/**
 * When a perfect site crossed over, if it crossed over recently enough to be news.
 *
 * Returns the timestamp of the run that turned it perfect, or null — either
 * because it did not just turn (the run before was already perfect, or there is
 * nothing to compare against) or because that happened too long ago to still be
 * news.
 *
 ...
 *
 * Shared so the stat bar's list and the NEW pill on the leaderboard cannot
 * disagree about which sites just turned.
 */
function newlyPerfectAt(points, cwvFailures, now) {
	const clean = points.filter((p) => !p.error);
	if (clean.length < 2) return null;
	if (pointIsPerfect(clean[clean.length - 2], cwvFailures)) return null;

	const timestamp = clean[clean.length - 1].t;
	return now - timestamp <= NEWLY_PERFECT_WINDOW_HOURS * 3600 * 1000 ? timestamp : null;
}
...
function buildStats(entries, { featuredOnly = false, now = Date.now() } = {}) {
	...
	const newlyPerfect = measured
		.filter((e) => perfect(e))
		.map((e) => {
			const timestamp = newlyPerfectAt(e.history ?? [], e.cwvFailures, now);
