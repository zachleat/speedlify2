 ...
 */
/**
 * Does this measurement keep the no-JS capture?
 *
 * True for every mode but "none" — see SCREENSHOT_MODES in lib/config.js. A
 * category turns pictures off to stop a thousand sites costing storage nobody
 * looks at, and "primary" is the middle setting: the Without JavaScript
 * comparison, without the filmstrip behind it.
 *
 * Full marks lifts even "none" to "primary": a site that just scored 400 is the
 * one everybody looks at — it can be the perfect site of the day, and that card
 * is a photograph. Self-limiting, since it only fires for sites that reached a
 * bar most never will.
 */
...
/**
 * Does this measurement keep the loading filmstrip?
 *
 * Stricter than `keepsScreenshots`, and separate from it because "primary" sits
 * between the two: that mode keeps the no-JS pair and throws the frames away.
 * The filmstrip is the expensive half — tens of kilobytes a site, rewritten
 * every week — so it is the half a category opts out of first.
 *
 * Configured mode only: full marks does not reach this far. The override exists
 * for the perfect-site card, and that card draws `ownScreenshot` — one viewport
 * capture, see statbar.njk — so a filmstrip was never what it needed. Granting
 * one meant a category set to "none" quietly accumulated strips for its perfect
 * scorers, which is a sample of nothing and cost the storage the setting was
 * chosen to avoid.
 */
export function keepsFilmstrip(site) {
	return (site?.screenshots ?? "filmstrip") === "filmstrip";
}
 ...
 */
export class Runner {
	constructor({ logger, runs = 3, formFactor = "mobile", cruxApiKey = null, axeEnabled = true, timeoutMs = 120_000, throttlingMethod = "simulate", filmstripThrottling = "devtools" }) {
		this.logger = logger;
		this.runs = runs;
		this.formFactor = formFactor;
		/*
		 * Lighthouse's default, "simulate", does not throttle the browser: it loads
		 * the page at full speed and models the metrics afterwards. "devtools"
		 * throttles for real, which is slower to run and the only way the filmstrip
		 * and the numbers describe the same load — see the note on `throttled` in
		 * lib/report.js.
		 */
		this.throttlingMethod = throttlingMethod;
		/*
		 * An additional run under this method supplies the filmstrip, while the
		 * scored runs above keep `throttlingMethod`.
		 *
		 * On by default, because a filmstrip captured unthrottled is a picture of a
		 * load nobody has: the frames would show a page arriving over the measuring
		 * machine's connection while the numbers beside them describe a slow one. A
		 * category that asked for a filmstrip asked for a useful one, so the
		 * `screenshots` mode is the only decision — this fires exactly where
		 * `keepsFilmstrip` is true.
		 *
		 * It costs an extra page load per site on top of `runs`, which is a real
		 * cost to somebody else's server. Null turns it off and leaves the strip
		 * coming from the median scored run, which is what it always did.
		 */
		this.filmstripThrottling = filmstripThrottling;
	...
	/*
	 * The form factor's config, with the throttling method laid over it.
	 *
	 * Left entirely alone at the default, so an unthrottled pass is byte-for-byte
	 * the run this project has always made — no config object where there used to
	 * be none, and no chance of the switch changing something else by accident.
	 */
	#lighthouseConfig(method = this.throttlingMethod) {
		const base = this.formFactor === "desktop" ? desktopConfig : undefined;
		if (method === "simulate") return base;

		return {
			...(base || { extends: "lighthouse:default" }),
			settings: { ...(base?.settings || {}), throttlingMethod: method },
		};
	}

	async #runOnce(url, method = this.throttlingMethod) {
		...
		const config = this.#lighthouseConfig(method);
		...
		return {
			...
			// The delay DevTools throttling adds to each response; zero when it is not applied.
			requestLatencyMs: method === "devtools" ? (result.lhr.configSettings?.throttling?.requestLatencyMs ?? 0) : 0,
		};
	}
	...
	async #measureOnce(site) {
		...
		/*
		 * One extra pass, throttled for real, kept only for its pictures.
		 *
		 * `throttlingMethod` is a property of a whole run: the metrics come out of
		 * whichever method produced them, so a run cannot be simulated for the
		 * scores and throttled for the frames. Two runs can. The scored runs above
		 * stay exactly as they always were — same method, same numbers, unbroken
		 * series — and this one is discarded except for the strip.
		 *
		 * Cheaper than switching the scored runs over, too: the throttling penalty
		 * is paid once per site rather than once per run.
		 */
		let strip = shots[attempts.indexOf(median)];
		let stripThrottling = this.throttlingMethod;
		let stripMarkers = null;
		let throttledBreakdown = null;

		if (this.filmstripThrottling && keepsFilmstrip(site)) {
			try {
				const extra = await this.#runOnce(url, this.filmstripThrottling);
				if (this.filmstripThrottling !== "simulate") {
					throttledBreakdown = throttledLcpBreakdown(extra.metrics?.lcpBreakdown, extra.requestLatencyMs);
				}
				if (extra.screenshots?.length) {
					strip = extra.screenshots;
					stripThrottling = this.filmstripThrottling;
					// Observed on the same trace as these frames, so they share a clock.
					stripMarkers = filmstripMarkers(extra.metrics?.timings, extra.requestLatencyMs);
				}
			} catch (err) {
				// The strip is a picture, not a measurement. A failed capture keeps
				// the scored run's frames rather than losing the whole site.
				this.logger.warn("throttled filmstrip pass failed, keeping the scored run's frames", {
					url,
					error: err.message,
				});
			}
		...
		if (axe?.interstitial) {
			...
			if (!again.axe?.interstitial) {
				 ...
				 *
				 * The captures go with them, so the previous ones stay on disk and
				 * the site page shows those instead of a picture of a waiting room.
				 */
				axe = challengedAxe(again.axe);
				pageShots = null;
				// The strip too: the Lighthouse runs can be challenged without looking it, and a null strip keeps the previous one on disk.
				strip = null;
				stripMarkers = null;
			}
		}
		...
		return {
			...
			screenshots: this.#filmstrip(site, strip, median),
			// Which method the frames above were captured under, which is not
			// necessarily the one the numbers came from — see the extra pass. Read
			// by ResultStore.write into the filmstrip manifest.
			screenshotThrottling: stripThrottling,
			screenshotMarkers: stripMarkers,
			...
			lab: median,
			// LCP subparts from the throttled filmstrip pass, where the scored runs' are read off a full-speed load.
			...(throttledBreakdown ? { throttledLcpBreakdown: throttledBreakdown } : {}),
			field,
			axe,
		};
	}
	...
	#filmstrip(site, frames, median) {
		if (!frames?.length || !keepsFilmstrip(site)) return null;
		return frames;
	}
	...
	async #axe(url, site, median) {
		...
		const result = await runAxe(url, {
			...
			screenshots: true,
			// The second capture, and the section it feeds, are kept for every mode
			// but "none": it doubles what is stored for the sites that asked for no
			// pictures at all, and "primary" is the setting that wants it without
			// the filmstrip. Except at full marks — see `keepsScreenshots`.
 ...
 */
/**
 * TTFB, FCP and LCP from the pass that captured the filmstrip, or null if it had none.
 *
 * Observed TTFB comes from Chrome's network timing, which DevTools throttling's
 * request latency never touches, so it is added back to land on the frames' clock.
 */
export function filmstripMarkers(timings, requestLatencyMs = 0) {
	const markers = {};
	for (let key of ["ttfb", "fcp", "lcp"]) {
		if (typeof timings?.[key] === "number") markers[key] = timings[key];
	}
	if (markers.ttfb !== undefined && requestLatencyMs > 0) markers.ttfb = Math.round(markers.ttfb + requestLatencyMs);
	return Object.keys(markers).length ? markers : null;
}

/**
 * LCP subparts from a DevTools-throttled pass, or null if it had none.
 *
 * The request latency is missing from observed TTFB for the same reason as in
 * `filmstripMarkers`, so it is moved there from the subpart that absorbed it and the total stays LCP.
 */
export function throttledLcpBreakdown(breakdown, requestLatencyMs = 0) {
	if (!breakdown || typeof breakdown.timeToFirstByte !== "number") return null;
	const out = { ...breakdown };
	if (requestLatencyMs > 0) {
		const next = ["resourceLoadDelay", "elementRenderDelay"].find((key) => typeof out[key] === "number");
		const moved = next ? Math.min(requestLatencyMs, out[next]) : 0;
		out.timeToFirstByte = Math.round((out.timeToFirstByte + moved) * 10) / 10;
		if (next) out[next] = Math.round((out[next] - moved) * 10) / 10;
	}
	return out;
}
...
export function asBotCheckFailure(record) {
	...
	const { lab, variance, screenshots, partialErrors, throttledLcpBreakdown, ...rest } = record;
