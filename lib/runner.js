...
export class Runner {
	 ...
	 */
	async measure(site) {
		return retryBotCheck((s) => this.#measureOnce(s), site, this.logger);
	}
...
/**
 * Run a measurement, and run it again if the first attempt got a bot check.
 *
 * Separated from the Runner so the sequencing can be tested without a browser:
 * the interesting part is how many attempts happen and which result is kept,
 * neither of which needs a real page load to verify.
 *
 * Returns the second attempt whenever there is one. Where the retry got through
 * that is the site; where it was challenged again the verdict is the same and
 * the numbers are fresher, so there is no reason to prefer the first.
 */
export async function retryBotCheck(attempt, site, logger = null) {
	const first = await attempt(site);
	if (!first?.axe?.interstitial) return first;

	logger?.warn?.("bot check, retrying once", { url: site.url, title: first.axe.interstitial });

	const second = await attempt(site);
	if (!second?.axe?.interstitial) logger?.info?.("retry got the site", { url: site.url });
