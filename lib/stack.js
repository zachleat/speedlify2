 ...
 * itself. Whatever the primary implies is left out, so Next.js is not also React.
 */
export function detectTools({ meta, metas, marks = [], markVersions = {} } = {}, headers = {}, primary = null) {
	...
	for (let detected of [primary, ...found]) {
		...
		for (let id of rule?.implies ?? []) implied.add(id);
	}

	return found.filter((t) => t.name !== primary?.name && !implied.has(t.id));
}
