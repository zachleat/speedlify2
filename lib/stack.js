...
const HOSTS = [
	...
	/*
	 * alwaysdata fronts customer sites with its own proxy, which is the only
	 * thing it stamps — the origin behind it still answers `server: Apache`, so
	 * without this the site reads as self-hosted.
	 */
	{ id: "alwaysdata", name: "alwaysdata", icon: "Alwaysdata", via: /\balproxy\b/i },
	...
	/*
	 * Google Web Server — what google.com itself answers with, and a different
	 * thing from the Google Frontend edge above: gws serves Google's own
	 * products rather than fronting somebody's project. Named for the company
	 * rather than expanded, since "Google Web Server" would read as a product
	 * anyone could be running.
	 *
	 * Anchored to the whole value. `gws` is three letters and would otherwise
	 * turn up inside unrelated server strings.
	 */
	{ id: "gws", name: "Google", icon: "Google", server: /^gws$/i },
	/*
	 * Static File Front End, which serves about.google and Google's other static
	 * properties. Named "Google" like `gws` above, so the two merge into one row
	 * in the stacks summary — that tally groups by name — rather than splitting
	 * one company across two arms of its own infrastructure.
	 */
	{ id: "sffe", name: "Google", icon: "Google", server: /^sffe$/i },
...
const GENERATORS = [
	...
	// `implies` names what a tool always brings, so detectTools does not list it
	// again as something the site also uses.
	{ id: "docusaurus", name: "Docusaurus", icon: "Docusaurus", meta: /docusaurus/i, mark: "docusaurus", implies: ["react"] },
	{ id: "hexo", name: "Hexo", icon: "Hexo", meta: /hexo/i },
	{ id: "gridsome", name: "Gridsome", icon: "Gridsome", meta: /gridsome/i, implies: ["vue"] },
	{ id: "quarto", name: "Quarto", icon: "Quarto", meta: /quarto/i },
	/*
	 * A document converter rather than a site generator, and it is what wrote the
	 * HTML — pandoc's standalone template stamps the tag itself.
	 *
	 * Deferred, like Silex above, for the opposite reason: pandoc is the layer
	 * *underneath* other tools rather than on top of one. Quarto renders through
	 * it, so a page can carry both names and both are true — and the wrapper is
	 * the more specific answer, whichever order the tags appear in. Deferring also
	 * keeps the version, on the rare tag that carries one; it would be pandoc's.
	 */
	{ id: "pandoc", name: "Pandoc", icon: "Pandoc", meta: /^pandoc\b/i, deferred: true },
	...
	{ id: "gatsby", name: "Gatsby", icon: "Gatsby", meta: /gatsby/i, mark: "gatsby", implies: ["react"] },
	...
	{ id: "vitepress", name: "VitePress", icon: "Vitepress", meta: /vitepress/i, mark: "vitepress", implies: ["vue"] },
	{ id: "vuepress", name: "VuePress", icon: "Vuedotjs", meta: /vuepress/i, implies: ["vue"] },
	...
	{ id: "framer", name: "Framer", icon: "Framer", meta: /^framer\b/i, implies: ["react"] },
	...
	// lose their icon.
	{ id: "nextjs", name: "Next.js", icon: "Nextdotjs", meta: /next\.js|^next\b/i, mark: "next", poweredBy: /next\.js/i, implies: ["react"] },
	{ id: "nuxt", name: "Nuxt", icon: "Nuxt", meta: /^nuxt/i, mark: "nuxt", implies: ["vue"] },
	/*
	...
	/*
	 * htmx, on the same terms as Vite above: not a generator, but for a
	 * server-rendered page that names no generator and carries no framework
	 * marker, "uses htmx" is the most specific true thing available — and unlike a
	 * bundler it is a choice the page makes visible in its own markup. ollama.com
	 * is the case: no generator tag, no framework marker, htmx 1.9.5.
	 *
	 * Listed after Vite, though in practice the two never compete: an htmx page
	 * already has an answer here, so lib/axe.js never spends the extra request
	 * that would confirm a Vite candidate.
	 */
	{ id: "htmx", name: "htmx", icon: "Htmx", mark: "htmx", fallback: true },
	{ id: "sveltekit", name: "SvelteKit", icon: "Svelte", meta: /sveltekit/i, mark: "sveltekit", implies: ["svelte"] },
	/*
	...
	{ id: "angular", name: "Angular", icon: "Angular", meta: /^angular/i, mark: "angular" },
	{ id: "ember", name: "Ember.js", icon: "Emberdotjs", meta: /^ember/i, mark: "ember" },
	{ id: "remix", name: "Remix", icon: "Remix", meta: /^remix/i, mark: "remix", implies: ["react", "reactrouter"] },
	{ id: "reactrouter", name: "React Router", icon: "Reactrouter", meta: /^react router/i, mark: "reactrouter", implies: ["react"] },
...
/**
 * Front-end frameworks, listed alongside the generator and never in its place.
 *
 * None of these says what built a page — an Eleventy site with a Preact island
 * is still an Eleventy site — so they stay out of `detectGenerator`, where a
 * match would also move a site out of any category that requires a generator.
 */
const FRAMEWORKS = [
	{ id: "react", name: "React", icon: "React", mark: "react" },
	{ id: "preact", name: "Preact", icon: "Preact", mark: "preact" },
	{ id: "vue", name: "Vue", icon: "Vuedotjs", mark: "vue" },
	{ id: "solid", name: "Solid", icon: "Solid", mark: "solid" },
	{ id: "lit", name: "Lit", icon: "Lit", mark: "lit" },
	{ id: "alpine", name: "Alpine.js", icon: "Alpinedotjs", mark: "alpine" },
];

/**
 * Libraries a page includes — components, icons — listed alongside the
 * generator like a framework.
 *
 * Kept apart from FRAMEWORKS because what a library pulls in is the library's
 * choice rather than the site's: 11ty.dev reached for Web Awesome, and Web
 * Awesome reached for Lit. `implies` is what says so, and without it the site
 * gets credited with a framework it never picked.
 */
const LIBRARIES = [
	{ id: "webawesome", name: "Web Awesome", icon: "Webawesome", mark: "webawesome", implies: ["lit"] },
	{ id: "fontawesome", name: "Font Awesome", icon: "Fontawesome", mark: "fontawesome" },
];
...
export function pageProbe() {
	...
	/*
	 * Versions a mark can supply on its own.
	 *
	 * Only Angular does, for now: it writes `ng-version` onto the root element,
	 * so a DOM detection there is as precise as a generator tag. Every other mark
	 * proves the framework and says nothing about which release.
	 */
	const markVersions = {};
	...
	/*
	 * Angular, from the attribute it stamps on the application root. The
	 * `_nghost-` / `_ngcontent-` pairs are the fallback: view encapsulation
	 * writes them onto every styled component, and they survive a build that
	 * somehow drops ng-version.
	 */
	const ngRoot = document.querySelector("[ng-version]");
	add("angular", ngRoot || document.querySelector("[ng-server-context], [_nghost], [_ngcontent]"));
	if (ngRoot) markVersions.angular = ngRoot.getAttribute("ng-version");

	/*
	 * Ember. Three independent tells, because the obvious one is gone: modern
	 * Ember does not put `Ember` on `window`, so a version is not available and
	 * `window.Ember` proves nothing either way.
	 *
	 * `.ember-application` is the class the framework puts on the element it
	 * boots into. The `<name>/config/environment` meta is Ember CLI's build
	 * signature — every app ships one, named after itself. `EmberENV` is the
	 * runtime's own configuration object.
	 */
	add(
		"ember",
		document.querySelector(".ember-application, .ember-view, meta[name$='/config/environment']") ||
			"EmberENV" in window,
	);
	...
	/*
	 * htmx. The version is the strong signal — the library writes it onto the
	 * global itself — and `hx-*` attributes are the fallback for a build that
	 * exports the module without ever touching `window`.
	 *
	 * Read through `version` rather than testing for `window.htmx`: an element
	 * with `id="htmx"` becomes `window.htmx` all on its own, and on a page about
	 * htmx that is exactly the id someone would reach for.
	 *
	 * `hx-on` is left out of the selector. htmx 2 spells it `hx-on:click`, which
	 * needs escaping in a selector and is always accompanied by one of these.
	 */
	const htmxVersion = typeof window.htmx?.version === "string" ? window.htmx.version : null;
	add(
		"htmx",
		htmxVersion ||
			document.querySelector(
				"[hx-get],[hx-post],[hx-put],[hx-patch],[hx-delete],[hx-boost],[hx-trigger],[hx-target],[hx-swap]," +
					"[data-hx-get],[data-hx-post],[data-hx-boost],[data-hx-trigger]",
			),
	);
	if (htmxVersion) markVersions.htmx = htmxVersion;

	/*
	 * React, Preact and Vue leave no global to test, but keep their bookkeeping on
	 * the DOM nodes they render under names no page would pick: React 16+ as
	 * `__reactFiber$<random>` or `_reactRootContainer`, Preact 10 as a `__k` vnode
	 * carrying its own mangled `__e` and `__v`, Vue 3 as `__vue_app__` on the
	 * mount point and Vue 2 as `__vue__`. Capped, since app roots sit near the top
	 * of the document and a long page is not worth a slow probe.
	 */
	const nodes = [document.documentElement, ...Array.prototype.slice.call(document.querySelectorAll("body, body *"), 0, 3000)]
		.filter(Boolean);
	let react = Boolean(document.querySelector("[data-reactroot]"));
	let preact = false;
	let vue = Boolean(document.querySelector("[data-v-app]"));
	let vueApp = null;
	for (let el of nodes) {
		for (let key of Object.keys(el)) {
			if (key.startsWith("__reactFiber$") || key.startsWith("__reactContainer$") || key === "_reactRootContainer") {
				react = true;
			} else if (key === "__vue_app__") {
				vueApp = el[key];
			} else if (key === "__vue__") {
				vue = true;
			} else if (key === "__k" && el.__k && typeof el.__k === "object" && "__e" in el.__k && "__v" in el.__k) {
				preact = true;
			}
		}
		// Scoped styles stamp `data-v-<hash>` into server-rendered markup, which
		// catches a Vue island that has not hydrated.
		if (!vue && el.getAttributeNames?.().some((name) => /^data-v-[0-9a-f]{8}$/.test(name))) vue = true;
	}
	add("react", react);
	add("preact", preact);
	add("vue", vue || vueApp);
	const vueVersion = vueApp?.version ?? window.Vue?.version;
	if (typeof vueVersion === "string") markVersions.vue = vueVersion;

	// Solid's hydration global, and the set its delegated events are kept in.
	add("solid", "_$HY" in window || document._$DX_DELEGATE);

	// Lit registers every copy of itself it loads, with the version.
	const litVersions = window.litHtmlVersions;
	add("lit", Array.isArray(litVersions) || Array.isArray(window.reactiveElementVersions));
	if (Array.isArray(litVersions) && litVersions.length) markVersions.lit = String(litVersions[0]);

	/*
	 * Font Awesome, in the three shapes it ships as. The SVG-with-JS runtime
	 * leaves its config and its own version on `window`. A kit or CDN asset is
	 * named in a tag, and that URL usually carries the version too. A
	 * self-hosted webfont build leaves neither, and then the classes are all
	 * there is — including v4's bare `fa`, which is only taken together with an
	 * `fa-` class beside it, since one letter pair is a class any page might use.
	 *
	 * Scripts and stylesheets only, not every `link`: a post *about* Font
	 * Awesome has the name in its own canonical URL, and 11ty.dev has exactly
	 * that page. What a page loads is the claim; what it links to is not.
	 */
	const faVersion = typeof window.FontAwesome?.version === "string" ? window.FontAwesome.version : null;
	const faAsset =
		[...document.querySelectorAll("script[src], link[rel~='stylesheet'][href]")]
			.map((el) => el.getAttribute("src") || el.getAttribute("href") || "")
			.find((url) => /(^|[/.])(kit\.fontawesome\.com|font-?awesome)/i.test(url)) || null;
	add(
		"fontawesome",
		faVersion ||
			faAsset ||
			...
			document.querySelector(
				".fa-solid, .fa-regular, .fa-light, .fa-thin, .fa-duotone, .fa-brands, .fa-sharp, " +
					".fas, .far, .fab, .fal, .fad, .svg-inline--fa, fa-icon, font-awesome-icon, " +
					"[class~='fa'][class*='fa-']",
			),
	);
	const faUrlVersion = faAsset?.match(/font-?awesome(?:-free|-pro)?[@/](\d+\.\d+(?:\.\d+)?)/i);
	if (faVersion || faUrlVersion) markVersions.fontawesome = faVersion ?? faUrlVersion[1];

	/*
	 * Web Awesome ships as ES module chunks and leaves nothing on `window`, so
	 * the mark is its elements: a `wa-` tag the registry has resolved. Writing
	 * the tag is not enough — an unregistered one is an unknown element, which
	 * is what a page copying the markup without the library would have.
	 *
	 * An unregistered tag still counts when the page names a Web Awesome asset
	 * to load, since a lazy loader like 11ty.dev's `<is-land on:visible
	 * import>` defers registration until the element scrolls into view.
	 */
	const waTags = nodes.filter((el) => el.localName?.startsWith("wa-"));
	add(
		"webawesome",
		waTags.some((el) => window.customElements?.get(el.localName)) ||
			(waTags.length > 0 &&
				[...document.querySelectorAll("script[src], link[href], [import]")].some((el) =>
					/web-?awesome/i.test(el.getAttribute("src") || el.getAttribute("href") || el.getAttribute("import") || ""),
				)),
	);

	// Read through `version` for the same reason as htmx above.
	const alpineVersion = typeof window.Alpine?.version === "string" ? window.Alpine.version : null;
	add("alpine", alpineVersion || document.querySelector("[x-data]"));
	if (alpineVersion) markVersions.alpine = alpineVersion;
	...
	// Parsed by `hiringStatus`; kept raw so a new reading applies to stored records.
	const hiring = document.querySelector('meta[name="hiring-status" i]')?.getAttribute("content")?.slice(0, 60) || null;

	return { meta: metas[0] ?? null, metas, marks, markVersions, viteCandidate, hiring, title: (document.title || "").slice(0, 200), bodyText };
}

/** Read `<meta name="hiring-status" content="open">` or `"open remote"` into a tag. */
export function hiringStatus(content) {
	const tokens = String(content ?? "").toLowerCase().trim().split(/\s+/);
	if (!tokens.includes("open")) return null;
	const remote = tokens.includes("remote");
	return { remote, label: remote ? "Open to Remote Work" : "Open to Work" };
}
 ...
 */
/**
 * The axe pass reduced to what is still true when it was served a bot check.
 *
 * The findings go: violations, passes and the rule list describe the challenge
 * page's markup, and `null` violations already means "not judged" everywhere
 * downstream — `axeBand` renders it as an unmeasured ring rather than a good
 * one, so nothing reads this as a clean bill of health.
 *
 * The response headers stay, and with them the detected host. A challenge is
 * served by the infrastructure sitting in front of the site, so "Vercel" or
 * "Cloudflare" is a true answer to "who serves this" even when the body is a
 * waiting room. The generator does not: that is read out of the page's own
 * markup, and the markup was theirs.
 */
export function challengedAxe(axe) {
	return {
		...axe,
		generator: null,
		probe: null,
...
export function detectGenerator({ meta, metas, marks = [], markVersions = {} } = {}, headers = {}) {
	...
	for (let g of GENERATORS) {
		...
		if (g.mark && marks.includes(g.mark)) {
			// A mark carries no version unless the page volunteered one — see
			// `markVersions` in pageProbe.
			const raw = markVersions?.[g.id] ?? null;
			return {
				id: g.id,
				name: g.name,
				icon: g.icon,
				version: raw ? versionFrom(raw) : null,
				source: "dom",
				raw: raw ?? candidates[0] ?? null,
			};
		}
	...
	// Last resort: the build tool or library a page is wearing, inferred from its
	// shape. Only reached when the site named no generator and carried no
	// framework marker.
	for (let g of GENERATORS) {
		if (g.fallback && g.mark && marks.includes(g.mark)) {
			// A version where the page volunteered one — htmx puts its own on the
			// global. Vite, the other fallback, never offers one.
			const raw = markVersions?.[g.id] ?? null;
			return {
				id: g.id,
				name: g.name,
...
/**
 * Everything else a page runs, beyond the one tool credited with building it.
 *
 * The same evidence `detectGenerator` reads, collected in full rather than
 * stopping at the first match — a second generator tag (Silex over Eleventy,
 * Quarto over Pandoc), a mark the primary does not account for (a Svelte island
 * on an Astro page) — plus the front-end frameworks.
 *
 * Only named rules count: an unknown second tag is usually a plugin advertising
 * itself.
 *
 * What another tool always brings is marked `implied` with that tool's name
 * rather than dropped — every Next.js site is a React site, which is worth
 * stating once on the site's own page and not worth a badge on 1,500
 * leaderboard rows. The callers decide; see `implies`.
 */
export function detectTools({ meta, metas, marks = [], markVersions = {} } = {}, headers = {}, primary = null) {
	const rules = [...GENERATORS, ...FRAMEWORKS, ...LIBRARIES];
	const found = [];
	const add = (rule, version, source) => {
		if (found.some((t) => t.name === rule.name)) return;
		found.push({ id: rule.id, name: rule.name, icon: rule.icon, version: version ?? null, source });
	};

	const candidates = metas?.length ? metas : meta ? [meta] : [];
	for (let candidate of candidates) {
		const rule = GENERATORS.find((g) => g.meta?.test(candidate));
		if (rule) add(rule, rule.secondary ? null : versionFrom(candidate), "meta");
	}

	for (let rule of rules) {
		if (!rule.mark || !marks.includes(rule.mark)) continue;
		const raw = markVersions?.[rule.id];
		add(rule, raw ? versionFrom(raw) : null, "dom");
	}

	const poweredBy = headers["x-powered-by"] || "";
	for (let rule of GENERATORS) {
		if (rule.poweredBy?.test(poweredBy)) add(rule, null, "header");
	}

	// Which tool implies which, so a page can say "comes with Next.js" rather
	// than listing React as a second discovery.
	const impliedBy = new Map();
	for (let detected of [primary, ...found]) {
		const rule = detected?.id ? rules.find((r) => r.id === detected.id) : null;
		for (let id of rule?.implies ?? []) if (!impliedBy.has(id)) impliedBy.set(id, detected.name);
	}

	return found
		.filter((t) => t.name !== primary?.name)
		.map((t) => (impliedBy.has(t.id) ? { ...t, implied: impliedBy.get(t.id) } : t));
}

/**
 * The part of a probe kept on the record, so tools can be re-read with rules
 * added later. Null for a probe that never read the page.
 */
export function storedProbe(probe, marks = probe?.marks) {
	if (!probe || probe.failed) return null;
	return {
		metas: (probe.metas ?? []).slice(0, 8).map((m) => String(m).slice(0, 120)),
		marks: marks ?? [],
		markVersions: probe.markVersions ?? {},
		// Only when present, so the thousands of records without one stay unchanged.
		...(probe.hiring ? { hiring: String(probe.hiring).slice(0, 60) } : {}),
	};
}
...
export function detectHost(headers = {}) {
	...
	/*
	 * A proxy that names its provider. Checked before `server` so an origin
	 * that only identifies itself here still beats the CDN in front of it, the
	 * same reason the request-id headers above go first.
	 */
	const via = headers.via || "";
	for (let h of HOSTS) {
		if (h.via && h.via.test(via)) {
			return { id: h.id, name: h.name, icon: h.icon, source: "via" };
		}
	}
