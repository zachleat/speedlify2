...
const FRAMEWORKS = [
	...
	// Without SvelteKit, as on huggingface.co; SvelteKit is the generator and implies it.
	{ id: "svelte", name: "Svelte", icon: "Svelte", mark: "svelte" },
	{ id: "htmx", name: "htmx", icon: "Htmx", mark: "htmx" },
];
...
export function pageProbe() {
	...
	// `/_astro/` is Astro's build-output directory, and the only tell on a
	// static page with no islands and scoped styles off (daily-dev-tips.com).
	add(
		"astro",
		document.querySelector(
			"astro-island, [class*='astro-'], script[src*='/_astro/'], link[href*='/_astro/']",
		),
	);
	 ...
	 * `__reactRouterVersion` alone is not enough: library mode sets it too, on
	 * single-page apps that only use the router and were built by something else.
	 *
	 * After `remix` so a site carrying both is named by the older, more specific
	 * one rather than by the library underneath it.
	 */
	add("reactrouter", "__reactRouterManifest" in window || "__reactRouterContext" in window);
	 ...
	 *
	 * Until then is-land also renames the element to `is-land--wa-copy-button`
	 * so it cannot upgrade early, which is the name the probe actually sees.
	 */
	const waNames = nodes.map((el) => el.localName?.match(/^(?:[a-z][\w.-]*--)?(wa-[\w.-]+)$/)?.[1]).filter(Boolean);
	add(
		"webawesome",
		waNames.some((name) => window.customElements?.get(name)) ||
			(waNames.length > 0 &&
...
export function detectGenerator({ meta, metas, marks = [], markVersions = {} } = {}, headers = {}) {
	...
	// Last resort: the build tool a page is wearing, inferred from its shape.
	// Only reached when the site named no generator and carried no framework marker.
	for (let g of GENERATORS) {
 ...
 * so the site's own page can say "comes with Next.js"; see `implies`.
 */
