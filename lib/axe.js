...
import { NODE_SAMPLES, clipSnippet } from "./metrics.js";
import { pageProbe, detectGenerator, detectHost, pickHostHeaders, detectInterstitial, storedProbe } from "./stack.js";
...
/**
 * Run the page's animations forward before the shutter, and how long to let
 * them run.
 *
 * A CSS reveal — content at `opacity: 0` that fades in — makes both halves of
 * the client-rendering test lie at once. The no-JS capture catches the page
 * mid-fade and reads as textless, and the two captures land at different points
 * of the same animation, so the difference between them measures the animation
 * rather than the JavaScript. stebre.ch is the case in point: Animate.css
 * `fadeInUp` on the intro and a one-second delay on the avatar, no script
 * involved anywhere, and the pair read as 1.5% text and 66.4% different — a
 * static Eleventy site filed as client-rendered.
 *
 * Set through CDP rather than by finishing animations from inside the page.
 * The no-JS capture has script execution disabled, and the two captures have to
 * be treated identically or the difference between them is not about scripts.
 *
 * A hundred turns the longest reveal anyone writes into a few milliseconds.
 * Infinite animations — spinners, ambient gradients — are left where they are,
 * spinning faster in a still photograph that never showed them moving.
 *
 * Settled, stebre.ch reads 34.6% text and 0% difference: the same page with and
 * without scripts, which is the right answer for a static site.
 */
const ANIMATION_PLAYBACK_RATE = 100;
const ANIMATION_SETTLE_MS = 100;

export async function fastForwardAnimations(page) {
	try {
		const client = await page.createCDPSession();
		await client.send("Animation.enable");
		await client.send("Animation.setPlaybackRate", { playbackRate: ANIMATION_PLAYBACK_RATE });
		await new Promise((resolve) => setTimeout(resolve, ANIMATION_SETTLE_MS));
	} catch {
		// A page that will not take the setting is photographed as it is. A
		// picture taken mid-fade is a worse measurement, not a failed one.
	}
}
 ...
 * anything depends on.
 */
export async function visualDifference(browser, a, b) {
...
/**
 * How much of the no-JS capture looks like text: the share of scanlines that
 * cross between light and dark many times, 0-100.
 *
 * The question is only ever "did this page say anything without scripts", and
 * every cheaper proxy answers a different one:
 *
 *  - Ink coverage measures how much is painted. ti.com's empty shell paints a
 *    gray panel and scores 10.7%; nuxt.com, which renders everything, scores 13.
 *  - Distinct colors measures decoration. mistral.ai draws a colorful logo over
 *    an empty body and scores 274.
 *  - Empty scanlines measures whitespace, not absence. It cannot tell an empty
 *    page from a roomy one: ibm.com is 74.2% empty rows and mistral.ai 74.1%,
 *    and one of them serves a headline and a paragraph.
 *
 * Text is what actually differs, and text has a signature no solid block or
 * gradient has: a row crossing a letter alternates light and dark a dozen times
 * or more within a few hundred pixels. Counting those rows finds prose without
 * reading it, which is all this needs — the alternative was OCR, several
 * megabytes of WASM to recover words we would then only count.
 *
 * The line falls in a gap: the sites that render nothing sit at 0-1.7% and the
 * ones that render something start at 5%.
 */
export async function textRows(browser, shot) {
	...
	try {
		...
		return await page.evaluate(
			async (src) => {
				...
				// Rec. 601 luma: a yellow logo on white is a weak edge to the eye and
				// should be a weak edge here too.
				const luma = (i) => (data[i] * 299 + data[i + 1] * 587 + data[i + 2] * 114) / 1000;

				let rows = 0;
				for (let y = 0; y < canvas.height; y++) {
					const row = y * canvas.width * 4;

					// Threshold at this row's own midpoint rather than at mid-gray.
					// Absolute mid-gray only finds dark text on a light page: npmx.dev
					// serves its content on a near-black background, every pixel in it
					// well under 128, and read as a blank page — 1.3% of rows against
					// the 7.3% this returns. The midpoint is polarity-blind, so light
					// text on dark counts exactly as dark text on light does.
					let min = 255;
					let max = 0;
					for (let x = 0; x < canvas.width; x++) {
						const v = luma(row + x * 4);
						if (v < min) min = v;
						if (v > max) max = v;
					}

					// A row of one flat color has a meaningless midpoint — sensor noise
					// either side of it would read as dozens of crossings. Below a real
					// spread there is nothing on this line worth thresholding.
					if (max - min < 40) continue;

					const mid = (min + max) / 2;
					let crossings = 0;
					let light = luma(row) > mid;

					for (let x = 1; x < canvas.width; x++) {
						const lit = luma(row + x * 4) > mid;
						if (lit !== light) crossings++;
						light = lit;
					}

					// Twelve crossings is roughly three words of body copy. A button, a
					// rule, or the edge of an image gives two or four.
					if (crossings >= 12) rows++;
				}

				return Math.round((rows / canvas.height) * 1000) / 10;
			},
...
async function probePage(page, attempts = 3) {
	...
	return { meta: null, metas: [], marks: [], viteCandidate: null, title: "", failed: true };
}
...
async function captureWithoutJavaScript(browser, url, formFactor, timeoutMs, scale) {
	...
	try {
		...
		/*
		 * How many words the page says without scripts, alongside the picture.
		 *
		 * The picture can only be measured in pixels, and pixels cannot tell an
		 * empty page from a roomy one — ibm.com's no-JS render is 74.2% blank rows
		 * and mistral.ai's is 74.1%, yet ibm serves a headline, a paragraph and
		 * two links while mistral serves the words "Contact sales". One of those
		 * is a site that works without JavaScript and one is a shell.
		 *
		 * Words, not characters: a page whose only text is a cookie banner and a
		 * skip link should not out-score one with a paragraph because its strings
		 * happen to be long. Read from the same load as the screenshot, so it
		 * costs nothing extra.
		 */
...
export async function runAxe(
	...
	try {
		 ...
		 */
		if (screenshots) {
			// Both captures get the same treatment, or the difference between them
			// stops being about scripts. See `fastForwardAnimations`.
			await fastForwardAnimations(page);
			...
			shots = {
				...
				noJsText: noJs ? await textRows(browser, noJs) : null,
			};
		}
		...
		if (!generator && probe.viteCandidate && (await confirmVite(page, probe.viteCandidate))) {
			marks = [...marks, "vite"];
			generator = detectGenerator({ ...probe, marks }, headers);
		}
		...
		return {
			...
			top: (results.violations || [])
				.map((v) => ({
					...
					samples: (v.nodes || []).slice(0, NODE_SAMPLES).map((node) => ({
						// Frames and shadow roots nest the target, one selector per boundary.
						selector: (node.target || []).map((t) => (Array.isArray(t) ? t.join(" ") : t)).join(" >>> ") || null,
						snippet: clipSnippet(node.html),
					})),
				}))
		 ...
		 *
		 * The interstitial check is here for the same reason as the stack
		 * detection, and it matters more: a bot check is a page that navigates out
		 * from under axe, so the pass failing is one of the ways a challenge shows
		 * up. Reading it only on success meant the sites most likely to be serving
		 * one were the sites least likely to be marked as serving one.
		 */
		return {
			shots,
			navigationTimedOut,
			generator: probe ? detectGenerator(probe, headers) : null,
			probe: storedProbe(probe),
			interstitial: probe ? detectInterstitial(probe) : null,
			host: detectHost(headers),
			headers,
			violations: null,
			passes: null,
