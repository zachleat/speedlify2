 ...
 *
 * 35% flags 183 of 2,979 readings (6.1%), against 290 (9.7%) at the 25% this
 * replaces. Unlike that figure, it sits on a slope rather than a plateau —
 * every point of threshold between 30% and 45% moves the count by roughly a
 * tenth — so it is a deliberate tolerance, not a natural break, and re-fitting
 * it to a later corpus is fair game.
 *
 * Both edges are live: two runs sit 43.0% and 39.8% under the median, so a
 * genuinely slow machine does trip this. The remaining 181 are fast ones.
 */
