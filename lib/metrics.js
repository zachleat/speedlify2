...
/**
 * Offending elements kept per failing rule, and the markup kept per element.
 * Results are committed for every run, so this is a sample to act on, not a full list.
 */
export const NODE_SAMPLES = 5;
const SNIPPET_CHARS = 200;

export function clipSnippet(html) {
...
/**
 * Failing audits for one category, by id and title rather than only the
 * weighted score. The score is coarse enough that you can add a whole broken
 * component and barely move it; the failing-audit list moves immediately.
 */
function categoryFailures(lhr, categoryId) {
	...
	for (let ref of refs) {
		...
		if (a.score < 1) {
			failing.push({
				id: ref.id,
				title: a.title,
				// The audit's share of the category score, so the list can be ordered
				// by what the failure actually costs.
				weight: num(ref.weight) ?? 0,
				...
				samples: (a.details?.items || [])
					.map((item) => item?.node)
					.filter((node) => node?.selector || node?.snippet)
					.slice(0, NODE_SAMPLES)
					.map((node) => ({
						selector: node.selector ?? null,
						snippet: clipSnippet(node.snippet),
	...
	return { failingCount: failing.length, applicableCount: applicable, failing };
}

/**
 * Accessibility failures, ordered by how much of the page they affect.
 */
function accessibility(lhr) {
	const out = categoryFailures(lhr, "accessibility");
	return {
		failingCount: out.failingCount,
		applicableCount: out.applicableCount,
		failingNodes: sum(out.failing, "nodes") ?? 0,
		failing: out.failing.sort((a, b) => (b.nodes ?? 0) - (a.nodes ?? 0)).slice(0, 15),
	};
}

/**
 * Best Practices and SEO failures. Both categories are a short list of pass/fail
 * checks, so the score alone never says which check broke — these do. Ordered by
 * weight: the first entry is the one costing the most points.
 */
function scoredCategory(lhr, categoryId) {
	const out = categoryFailures(lhr, categoryId);
	return {
		failingCount: out.failingCount,
		applicableCount: out.applicableCount,
		failing: out.failing.sort((a, b) => b.weight - a.weight).slice(0, 15),
	};
}
...
export function extractMetrics(lhr) {
	return {
		...
		bestPractices: scoredCategory(lhr, "best-practices"),
		seo: scoredCategory(lhr, "seo"),
		hygiene: hygiene(lhr),
...
/*
 * Where the picture stopped changing, to the eye rather than to the byte.
 *
 * Frames are stored under a hash of their own bytes, so "the same frame" is
 * exact — and exact is the wrong test here. A carousel sampled every 375ms never
 * lands byte-identical twice, so a page that finished loading and left one
 * widget moving reads as never finishing at all. Measured across the corpus: 61
 * of 520 strips end while still "changing", and not one of them repeats a frame,
 * so no amount of comparing filenames will find them.
 *
 * Each frame is reduced to mean luma over a coarse grid, which averages JPEG
 * noise away, and two frames count as the same picture when under a tenth of
 * those blocks have moved.
 *
 * Both constants were measured rather than chosen. Share of blocks differing
 * from the final frame, for pages that had plainly finished — samsung.com 2.5%,
 * duda.co 6-8%, webdriver.io 5-10% — against pages that plainly had not —
 * sap.com 9-18%, palantir.com 28-46%, squarespace.com 67-70%. The two groups
 * separate either side of a tenth.
 *
 * Deliberately blind to a change confined to a small region, which is the same
 * insensitivity that stops a spinner holding a page open to the end of the
 * trace.
 */
const SETTLE_COLS = 16;
const SETTLE_ROWS = 32;
/** Luma steps, of 255, before a block counts as having moved. */
const SETTLE_BLOCK_DELTA = 8;
/** Share of blocks that may move with the frames still counting as the same. */
const SETTLE_CHANGED_SHARE = 0.1;

function lumaGrid(buffer) {
	const { data, width, height } = jpeg.decode(buffer, { useTArray: true });
	const cells = new Float64Array(SETTLE_COLS * SETTLE_ROWS);
	const counts = new Uint32Array(SETTLE_COLS * SETTLE_ROWS);

	for (let y = 0; y < height; y++) {
		const row = Math.min(SETTLE_ROWS - 1, Math.floor((y / height) * SETTLE_ROWS));
		for (let x = 0; x < width; x++) {
			const col = Math.min(SETTLE_COLS - 1, Math.floor((x / width) * SETTLE_COLS));
			const i = (y * width + x) * 4;
			cells[row * SETTLE_COLS + col] +=
				0.2126 * data[i] + 0.7152 * data[i + 1] + 0.0722 * data[i + 2];
			counts[row * SETTLE_COLS + col] += 1;
		}
	}

	for (let i = 0; i < cells.length; i++) cells[i] /= counts[i] || 1;
	return cells;
}
...
function sameGrid(a, b) {
	...
	for (let i = 0; i < a.length; i++) {
		if (Math.abs(a[i] - b[i]) > SETTLE_BLOCK_DELTA) changed += 1;
	}
	return changed / a.length <= SETTLE_CHANGED_SHARE;
}

/**
 * Timing of the earliest frame the page never meaningfully changed after, or
 * null if the frames cannot be read. Never later than a comparison of the bytes
 * would give: identical bytes are identical pictures.
 */
export function settleTime(frames) {
	if (!frames?.length) return null;

	let grids;
	try {
		grids = frames.map((frame) => lumaGrid(frame.buffer));
	} catch {
		// A frame we cannot decode is not a reason to lose the measurement. The
		// pages fall back to comparing stored filenames.
		return null;
	}

	const last = grids[grids.length - 1];
	let at = frames[frames.length - 1].timing ?? null;
	for (let i = grids.length - 1; i >= 0; i--) {
		if (!sameGrid(grids[i], last)) break;
		at = frames[i].timing ?? at;
	}
	return at;
}
